...
| Wave 4 / 晋级或负结果记录 | **done**（无候选晋级；负结果记录追加进 `strategy-iteration-progress-2026-07-01.md`） | `f44576c` |
...
状态：**done（负结果，但有正向信号）**。正式报告：`reports/research/control/step10-w3-combination-2026-09.md`。评估脚本（已入库）：`scripts/evaluate_step10_wave3_combination.py`。证据 JSON（本地，未入库）：`reports/research/control/step10-w3-combination-2026-09.json`。

### 做了什么

- 按计划 §6 预注册规则：从 F1/F2/W2 三个族各取"探路遍"里 Sharpe-ex-BIL 最高的候选（F1=C12 0.589，F2=lookback6/top5/equal 0.648，W2=top_fraction0.10/月度 0.474），三者都过 ≥0.4 门槛。两两 OOS 日收益相关性（1172 个公共交易日，2022-01-03 至 2026-09-03）：F1-F2=0.410、F1-W2=0.392、F2-W2=**0.710**（超过 ≤0.5 门槛）。三者不能同时入选。
- 计划文本没有钉死"三者互斥时怎么选"，本次按 Sharpe 从高到低贪心添加（先接受 F2，再接受 F1 因为与 F2 相关性合格，最后拒绝 W2 因为与已入选的 F2 相关性 0.710 超标），取舍规则在报告与脚本 docstring 里都写明,不是试出来的。另有 4 处类似的、计划文本未完全钉死的执行判断（DSR 试验数范围、逆波动率回看期启动方式、fold 重建方式、组合层成本假设）均在报告"执行判断"一节逐条写明理由。
- 权重：逆波动率、月末重算，58 次权重更新（1 次启动 + 57 个月末），F1 权重区间 0.367-0.899、F2 权重区间 0.101-0.633，固定权重直到下次月末、逐日盯市，未做优化。
- 评估：复用 `mechanism_eval.evaluate_candidate`（新合同，基准 SPY），`oos_fold_returns` 按日历年（2022-2026）直接从组合自身的日收益序列切分重建（原因：组合的输入本身已经是三个 sleeve 各自的"样本外"拼接流，没有更早的训练区间可供 `rolling_origin_folds` 使用，也不需要——逆波动率加权是固定规则，不在训练窗口上拟合任何参数）；DSR 试验数 = 三个族有效 N 之和 + 1 = 4（保守口径：三个"参与角逐"的族都算，不只是最终入选的两个）。

### 结果

- **组合不通过新合同**：8 门过 6，`cagr_excess_vol_matched_benchmark`（+2.44pp，门槛 5pp）与 `sharpe_excess_bil`（0.743，门槛 1.0）仍不过。
- **但组合在这两道门槛上都比任一单 sleeve 更接近**：Sharpe-ex-BIL 从 F1 的 0.589、F2 的 0.648 提升到 0.743；超额 CAGR 从 F1 的 +0.99pp、F2 的 +1.33pp 提升到 +2.44pp；`max_drawdown`（-9.89%）与 `mar`（1.206）也都是三者中最好的。这是本轮唯一一次看到"分散化把候选往门槛方向推"的直接证据，只是这次推得还不够远。
- 危机窗口诊断：组合样本从 2022-01-04 起，2020 疫情崩盘/2018Q4 都在窗口之前（如实标注 no overlap，非遗漏）；2022 全年组合 -8.40% vs SPY -18.64%。选择后窗口（41 天）组合 +8.49% vs SPY +25.21%，跑输但好于 F1 单独同期的 -8.23%。
- 对下一轮的含义：这条路线要通过，需要再补一个与 F1/F2 相关性都 ≤0.5、质量不差于 W2 的第三个 sleeve（W2 被排除是因为和 F2 相关性太高，不是因为它本身弱）。
