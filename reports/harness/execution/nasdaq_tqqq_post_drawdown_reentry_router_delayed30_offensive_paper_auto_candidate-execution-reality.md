# Execution Reality — nasdaq_tqqq_post_drawdown_reentry_router_delayed30_offensive_paper_auto_candidate
...
## Rationale
Recommended loo_limit (Limit-on-open with explicit limit offset; cancel if not filled.). Leveraged ETF universe → price protection prioritised over fill certainty. Daily next-bar-open execution → opening-auction routing preferred. paper_auto mode → must compare at least two methods.
