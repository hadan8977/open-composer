...
from open_composer.research.pit_semantic_theme_r18 import (
    ...
    build_r18_d01_targets,
    build_r18_feature_dataset,
    build_segment_targets,
    development_folds,
    load_and_validate_r18_specs,
    load_r18_price_panel,
    training_rows_for_r18_prediction,
)
...
def test_r18_specs_bind_fixed_states_classifiers_and_exact_fallback() -> None:
    specs = load_and_validate_r18_specs(ROOT)

    assert set(specs) == set(SPEC_PATHS)
    assert (
        specs["R18D01"].portfolio.selected_route_label
        == "pit_semantic_theme_r18:D01:stress_recovery_boost_v1"
    )
    route_contract = specs["R18D01"].notes.model_dump(mode="json")["route_contract"]
    assert route_contract["stress_target"] == {"QQQ": 1.0}
    assert route_contract["stress_recovery"] == _stress_recovery_contract()
    assert specs["R18M01"].model is not None
    assert specs["R18M01"].model.kind == "logistic_regression_classifier"
    assert specs["R18M01"].model.label.horizon_bars == 20
    assert specs["R18M01"].model.training.embargo_bars == 21
    assert specs["R18M01"].model.training.retrain_every_bars == 20
    assert specs["R18M01"].model.training.seed == 15101
    assert specs["R18M02"].model is not None
    assert specs["R18M02"].model.kind == "lightgbm_classifier"
    assert specs["R18M02"].model.label.horizon_bars == 10
    assert specs["R18M02"].model.selection.threshold == 0.20
    assert specs["R18M02"].model.training.seed == 15102
    assert specs["R18F01"].model is not None
    assert specs["R18F01"].model.model_dump(mode="json") == specs["R18M01"].model.model_dump(
        mode="json"
    )
    assert tuple(specs["R18M01"].model.features) == MODEL_FEATURES
    assert tuple(specs["R18M02"].model.features) == MODEL_FEATURES
    assert "qqq_trend_gap_50" not in MODEL_FEATURES
    assert "tqqq_momentum_10" not in MODEL_FEATURES


def test_r18_search_space_matches_specs_and_cumulative_trial_contract() -> None:
    iteration = ROOT / "reports/research/iterations/mom_pit_semantic_theme_r18"
    ...
    assert search["cumulative_trial_count"] == 8094
    assert cumulative["prior_effective_trial_count"] == 8086
    assert cumulative["effective_trial_count"] == 8094


def test_r18_clean_sip_panel_removes_r11_split_discontinuity() -> None:
    panel = load_r18_price_panel(ROOT)
...
def test_r18_folds_are_four_nonoverlapping_embargoed_full_year_windows() -> None:
    panel = load_r18_price_panel(ROOT)
...
def test_r18_features_labels_and_embargo_use_registered_timing() -> None:
    panel = load_r18_price_panel(ROOT)
    specs = load_and_validate_r18_specs(ROOT)
    ...
    leadership_return = (
        LEADERSHIP_WEIGHTS["USD"] * usd_return + LEADERSHIP_WEIGHTS.get("TQQQ", 0.0) * tqqq_return
    )
...
def test_r18_models_fit_classification_labels_without_future_data() -> None:
    specs = load_and_validate_r18_specs(ROOT)
        ...
        rows.append(
            {
                ...
                **{
                ...
                "m01_usd100_label": float(position % 3 == 0),
    ...
    assert set(fitted) == {"R18M01", "R18M02"}
...
def test_r18_frame_hash_binds_session_identity_and_both_label_horizons() -> None:
    ...
    original = _r18_frame_hash(frame, list(MODEL_FEATURES))
    changed = frame.copy()
    changed.loc[0, "m02_label_end_position"] = 22

    assert _r18_frame_hash(frame, list(MODEL_FEATURES)) == original
    assert _r18_frame_hash(changed, list(MODEL_FEATURES)) != original


def test_r18_d01_uses_scheduled_usd100_and_immediate_stress_recovery() -> None:
    ...
    targets, records = build_r18_d01_targets(panel, specs["R18D01"], dataset)

    assert [row["selected_target"] for row in records[:10]] == ["TQQQ"] * 10
    assert [row["selected_target"] for row in records[10:14]] == ["USD100"] * 4
...
def test_r18_m01_authorizes_only_usd100_and_m02_exits_to_qqq(monkeypatch) -> None:
    ...
    def fake_probability(model, current):
        column = "m01_probability" if model.candidate_id == "R18M01" else "m02_probability"
        return float(current.iloc[0][column])

    monkeypatch.setattr(r18, "_fit_route_models", fake_fit)
    monkeypatch.setattr(r18, "_predict_probability", fake_probability)
    ...
    m01 = result.targets["R18M01"]
    m02 = result.targets["R18M02"]
    ...
    assert result.targets["R18F01"].equals(m01)
    assert all(
    ...
    m01_predictions = [
        row["probability"] for row in result.prediction_records if row["candidate_id"] == "R18M01"
    ]
    f01_predictions = [
        row["probability"] for row in result.prediction_records if row["candidate_id"] == "R18F01"
    ]
    assert f01_predictions == m01_predictions


def test_r18_ml_routes_inherit_immediate_stress_recovery(monkeypatch) -> None:
    ...
    monkeypatch.setattr(r18, "_fit_route_models", fake_fit)
    monkeypatch.setattr(r18, "_predict_probability", lambda model, current: 0.90)
...
def test_r18_fold_evaluation_uses_shared_terminal_metric_views() -> None:
    ...
    result = _evaluate_folds(opens, folds, {"R18TEST": targets})["R18TEST"][0]
