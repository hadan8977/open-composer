...
from open_composer.research.pit_semantic_theme_r22 import (
    ...
    training_rows_for_r22_prediction,
)
...
def test_r22_specs_bind_fixed_states_classifiers_and_exact_fallback() -> None:
    specs = load_and_validate_r22_specs(ROOT)

    assert set(specs) == set(SPEC_PATHS)
    assert (
        specs["R22D01"].portfolio.selected_route_label
        == "pit_semantic_theme_r22:D01:usd_pressure_dual_fast_recovery_v1"
    )
    route_contract = specs["R22D01"].notes.model_dump(mode="json")["route_contract"]
    ...
    assert (
        route_contract["usd_pressure_override"][
            "pressure_recovery_ignores_slow_breadth_confirmation"
        ]
        is True
    )
    assert specs["R22M01"].model is not None
    assert specs["R22M01"].model.kind == "logistic_regression_classifier"
    assert specs["R22M01"].model.label.horizon_bars == 20
    assert specs["R22M01"].model.training.embargo_bars == 21
    assert specs["R22M01"].model.training.retrain_every_bars == 20
    assert specs["R22M01"].model.training.seed == 15101
    assert specs["R22M02"].model is not None
    assert specs["R22M02"].model.kind == "lightgbm_classifier"
    assert specs["R22M02"].model.label.horizon_bars == 10
    assert specs["R22M02"].model.selection.threshold == 0.20
    assert specs["R22M02"].model.training.seed == 15102
    assert specs["R22F01"].model is not None
    assert specs["R22F01"].model.model_dump(mode="json") == specs["R22M01"].model.model_dump(
        mode="json"
    )
    assert tuple(specs["R22M01"].model.features) == MODEL_FEATURES
    assert tuple(specs["R22M02"].model.features) == MODEL_FEATURES
    assert "qqq_trend_gap_50" not in MODEL_FEATURES
    assert "tqqq_momentum_10" not in MODEL_FEATURES


def test_r22_search_space_matches_specs_and_cumulative_trial_contract() -> None:
    iteration = ROOT / "reports/research/iterations/mom_pit_semantic_theme_r22"
    ...
    assert search["cumulative_trial_count"] == 8126
    assert cumulative["prior_effective_trial_count"] == 8118
    assert cumulative["effective_trial_count"] == 8126


def test_r22_clean_sip_panel_removes_r11_split_discontinuity() -> None:
...
def test_r22_folds_are_four_nonoverlapping_embargoed_full_year_windows() -> None:
    panel = load_r22_price_panel(ROOT)
...
def test_r22_features_labels_and_embargo_use_registered_timing() -> None:
    panel = load_r22_price_panel(ROOT)
    specs = load_and_validate_r22_specs(ROOT)
    dataset = build_r22_feature_dataset(panel)
    _, d01_records = build_r22_d01_targets(panel, specs["R22D01"], dataset)
    folds = development_folds(panel.open.index)
...
def test_r22_models_fit_classification_labels_without_future_data() -> None:
    specs = load_and_validate_r22_specs(ROOT)
    ...
    assert set(fitted) == {"R22M01", "R22M02"}
...
def test_r22_frame_hash_binds_session_identity_and_both_label_horizons() -> None:
    ...
    original = _r22_frame_hash(frame, list(MODEL_FEATURES))
    changed = frame.copy()
    changed.loc[0, "m02_label_end_position"] = 22

    assert _r22_frame_hash(frame, list(MODEL_FEATURES)) == original
    assert _r22_frame_hash(changed, list(MODEL_FEATURES)) != original


def test_r22_usd_pressure_latches_at_frozen_tail_boundary_until_trend_recovery() -> None:
...
def test_r22_pressure_recovery_uses_fast_prices_without_slow_breadth() -> None:
    dataset = build_r22_feature_dataset(load_r22_price_panel(ROOT))
    ...
    assert (pressure & fast_price & ~breadth & dataset["recovery_boost"]).any()
    assert (ordinary_stress & fast_price & ~breadth & ~dataset["recovery_boost"]).any()


def test_r22_d01_uses_scheduled_usd100_and_immediate_stress_recovery() -> None:
    ...
    targets, records = build_r22_d01_targets(panel, specs["R22D01"], dataset)
...
def test_r22_m01_authorizes_only_tqqq_override_and_m02_exits_to_qqq(monkeypatch) -> None:
    ...
    def fake_probability(model, current):
        column = "m01_probability" if model.candidate_id == "R22M01" else "m02_probability"
        return float(current.iloc[0][column])

    monkeypatch.setattr(r22, "_fit_route_models", fake_fit)
    monkeypatch.setattr(r22, "_predict_probability", fake_probability)
    ...
    m01 = result.targets["R22M01"]
    m02 = result.targets["R22M02"]
    ...
    assert result.targets["R22F01"].equals(m01)
    assert all(
    ...
    m01_predictions = [
        row["probability"] for row in result.prediction_records if row["candidate_id"] == "R22M01"
    ]
    f01_predictions = [
        row["probability"] for row in result.prediction_records if row["candidate_id"] == "R22F01"
    ]
    assert f01_predictions == m01_predictions


def test_r22_ml_routes_inherit_immediate_stress_recovery(monkeypatch) -> None:
    ...
    monkeypatch.setattr(r22, "_fit_route_models", fake_fit)
    monkeypatch.setattr(r22, "_predict_probability", lambda model, current: 0.90)
...
def test_r22_fold_evaluation_uses_shared_terminal_metric_views() -> None:
    ...
    result = _evaluate_folds(opens, folds, {"R22TEST": targets})["R22TEST"][0]
