...
def test_r14_specs_bind_fixed_states_classifiers_and_exact_fallback() -> None:
    specs = load_and_validate_r14_specs(ROOT)

    assert set(specs) == set(SPEC_PATHS)
    assert (
        specs["R14D01"].portfolio.selected_route_label
        == "pit_semantic_theme_r14:D01:stable_leverage_state_v1"
    )
    assert specs["R14M01"].model is not None
    assert specs["R14M01"].model.kind == "logistic_regression_classifier"
    assert specs["R14M01"].model.label.horizon_bars == 20
    assert specs["R14M01"].model.training.embargo_bars == 21
    assert specs["R14M01"].model.training.retrain_every_bars == 20
    assert specs["R14M01"].model.training.seed == 14101
    assert specs["R14M02"].model is not None
    assert specs["R14M02"].model.kind == "lightgbm_classifier"
    assert specs["R14M02"].model.label.horizon_bars == 10
    assert specs["R14M02"].model.selection.threshold == 0.20
    assert specs["R14M02"].model.training.seed == 14102
    assert specs["R14F01"].model is not None
    assert specs["R14F01"].model.model_dump(mode="json") == specs["R14M01"].model.model_dump(
        mode="json"
    )
    assert tuple(specs["R14M01"].model.features) == MODEL_FEATURES
    assert tuple(specs["R14M02"].model.features) == MODEL_FEATURES


def test_r14_clean_sip_panel_removes_r11_split_discontinuity() -> None:
...
def test_r14_folds_are_four_nonoverlapping_embargoed_full_year_windows() -> None:
    panel = load_r14_price_panel(ROOT)
...
def test_r14_models_fit_classification_labels_without_future_data() -> None:
        ...
        rows.append(
            {
                ...
                **{
                ...
                "m01_barbell_label": float(position % 3 == 0),
    ...
    assert set(fitted) == {"R14M01", "R14M02"}
...
def test_r14_frame_hash_binds_session_identity_and_both_label_horizons() -> None:
    ...
    original = _r14_frame_hash(frame, list(MODEL_FEATURES))
    changed = frame.copy()
    changed.loc[0, "m02_label_end_position"] = 22

    assert _r14_frame_hash(frame, list(MODEL_FEATURES)) == original
    assert _r14_frame_hash(changed, list(MODEL_FEATURES)) != original


def test_r14_d01_uses_scheduled_barbell_and_immediate_qqq_stress() -> None:
    ...
    targets, records = build_r14_d01_targets(panel, specs["R14D01"], dataset)

    assert [row["selected_target"] for row in records[:10]] == ["TQQQ"] * 10
    assert [row["selected_target"] for row in records[10:14]] == ["SOXL80_TQQQ20"] * 4
    assert [row["selected_target"] for row in records[14:]] == ["QQQ", "QQQ"]
    assert targets.iloc[9]["TQQQ"] == 1.0
    assert targets.iloc[10]["SOXL"] == 0.8
    assert targets.iloc[10]["TQQQ"] == 0.2
    assert targets.iloc[14]["QQQ"] == 1.0
    assert targets.loc[:, ["BIL", "GLD"]].eq(0.0).all().all()
    assert targets.sum(axis=1).eq(1.0).all()


def test_r14_m01_authorizes_only_barbell_and_m02_exits_to_qqq(monkeypatch) -> None:
    ...
    def fake_probability(model, current):
        column = "m01_probability" if model.candidate_id == "R14M01" else "m02_probability"
        return float(current.iloc[0][column])

    monkeypatch.setattr(r14, "_fit_route_models", fake_fit)
    monkeypatch.setattr(r14, "_predict_probability", fake_probability)
    ...
    m01 = result.targets["R14M01"]
    m02 = result.targets["R14M02"]
    assert m01.iloc[:10]["TQQQ"].eq(1.0).all()
    assert m01.iloc[10:]["SOXL"].eq(0.8).all()
    assert m01.iloc[10:]["TQQQ"].eq(0.2).all()
    assert m02.iloc[:11].equals(m01.iloc[:11])
    assert m02.iloc[11:]["QQQ"].eq(1.0).all()
    assert result.targets["R14F01"].equals(m01)
    assert all(
    ...
    m01_predictions = [
        row["probability"] for row in result.prediction_records if row["candidate_id"] == "R14M01"
    ]
    f01_predictions = [
        row["probability"] for row in result.prediction_records if row["candidate_id"] == "R14F01"
    ]
    assert f01_predictions == m01_predictions


def test_r14_fold_evaluation_uses_shared_terminal_metric_views() -> None:
    ...
    result = _evaluate_folds(opens, folds, {"R14TEST": targets})["R14TEST"][0]
