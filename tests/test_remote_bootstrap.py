...
def test_vps_bootstrap_builds_dry_run_plan_without_raw_secret(sample_workspace: Path) -> None:
    ...
    assert config.plan.cleanup_vercel is True
    assert any(step.name == "vercel_cleanup" and step.status == "ok" for step in config.plan.steps)
...
def test_detect_public_ip_uses_probe_fallback(monkeypatch) -> None:
    calls: list[str] = []
    ...
    monkeypatch.setattr("open_composer.remote.bootstrap.urllib.request.urlopen", fake_urlopen)

    assert detect_public_ip() == "203.0.113.45"
    assert calls[:2] == ["https://api.ipify.org", "https://ifconfig.me/ip"]
...
def test_vps_bootstrap_apply_uses_vercel_env_and_redacts_token(
    ...
    assert all("--value" not in call[0] and "--yes" in call[0] for call in env_adds)
    ...
    assert all(call[1] is not None for call in env_adds)
    assert "vercel_token_secret" not in (
        sample_workspace / "reports" / "deployment" / "vps-bootstrap" / "plan.json"
    ).read_text(encoding="utf-8")


def test_vps_bootstrap_apply_cleans_up_stale_vercel_deployments(
    sample_workspace: Path, monkeypatch
) -> None:
    calls: list[list[str]] = []
    (sample_workspace / "dashboard").mkdir()
    ...
    assert ["vercel", "remove", "open-composer-dashboard", "--safe", "--yes"] in calls
    assert any(step.name == "apply.vercel_cleanup" and step.status == "ok" for step in plan.steps)


def test_vps_bootstrap_cleanup_ok_when_no_stale_vercel_deployments(
    sample_workspace: Path, monkeypatch
) -> None:
    (sample_workspace / "dashboard").mkdir()
    monkeypatch.setattr("open_composer.remote.bootstrap.is_tcp_port_available", lambda port: True)
    ...
    def fake_runner(
        ...
        if args[:2] == ["vercel", "remove"]:
            return CommandExecutionResult(
                args=list(args),
                returncode=1,
                stdout=(
                    'Fetching deployment(s) "open-composer-dashboard"\n'
                    "> Could not find unaliased deployments or projects matching "
    ...
    assert plan.status == "ok"
    assert any(
        step.name == "apply.vercel_cleanup"
        and step.status == "ok"
        and "no stale deployments" in step.message
        for step in plan.steps
    )


def test_vps_bootstrap_apply_creates_missing_vercel_project(
    sample_workspace: Path, monkeypatch
) -> None:
    calls: list[list[str]] = []
    (sample_workspace / "dashboard").mkdir()
    ...
    def fake_runner(
        ...
        calls.append(list(args))
        if args[:3] == ["vercel", "project", "inspect"]:
            return CommandExecutionResult(
                args=list(args),
                returncode=1,
                stdout="",
    ...
    assert ["vercel", "link", "--yes", "--project", "open-composer-dashboard"] in calls
...
def test_vps_bootstrap_restarts_existing_remote_service(sample_workspace: Path) -> None:
    ...
    config = build_vps_bootstrap_config(
        ...
        skip_prepare=True,
        verify=False,
        systemd_unit_path=sample_workspace / "open-composer-remote.service",
