...
from open_composer.research.pit_semantic_theme_r21 import (
    ...
    build_r21_d01_targets,
    build_r21_feature_dataset,
    build_segment_targets,
    development_folds,
    load_and_validate_r21_specs,
    load_r21_price_panel,
    training_rows_for_r21_prediction,
)
...
def test_r21_specs_bind_fixed_states_classifiers_and_exact_fallback() -> None:
    specs = load_and_validate_r21_specs(ROOT)

    assert set(specs) == set(SPEC_PATHS)
    assert (
        specs["R21D01"].portfolio.selected_route_label
        == "pit_semantic_theme_r21:D01:usd_pressure_fast_recovery_v1"
    )
    route_contract = specs["R21D01"].notes.model_dump(mode="json")["route_contract"]
    ...
    assert route_contract["usd_pressure_override"]["suppresses_tqqq_recovery"] is False
    assert specs["R21M01"].model is not None
    assert specs["R21M01"].model.kind == "logistic_regression_classifier"
    assert specs["R21M01"].model.label.horizon_bars == 20
    assert specs["R21M01"].model.training.embargo_bars == 21
    assert specs["R21M01"].model.training.retrain_every_bars == 20
    assert specs["R21M01"].model.training.seed == 15101
    assert specs["R21M02"].model is not None
    assert specs["R21M02"].model.kind == "lightgbm_classifier"
    assert specs["R21M02"].model.label.horizon_bars == 10
    assert specs["R21M02"].model.selection.threshold == 0.20
    assert specs["R21M02"].model.training.seed == 15102
    assert specs["R21F01"].model is not None
    assert specs["R21F01"].model.model_dump(mode="json") == specs["R21M01"].model.model_dump(
        mode="json"
    )
    assert tuple(specs["R21M01"].model.features) == MODEL_FEATURES
    assert tuple(specs["R21M02"].model.features) == MODEL_FEATURES
    assert "qqq_trend_gap_50" not in MODEL_FEATURES
    assert "tqqq_momentum_10" not in MODEL_FEATURES


def test_r21_search_space_matches_specs_and_cumulative_trial_contract() -> None:
    iteration = ROOT / "reports/research/iterations/mom_pit_semantic_theme_r21"
    ...
    assert search["cumulative_trial_count"] == 8118
    assert cumulative["prior_effective_trial_count"] == 8110
    assert cumulative["effective_trial_count"] == 8118


def test_r21_clean_sip_panel_removes_r11_split_discontinuity() -> None:
    panel = load_r21_price_panel(ROOT)
...
def test_r21_folds_are_four_nonoverlapping_embargoed_full_year_windows() -> None:
    panel = load_r21_price_panel(ROOT)
...
def test_r21_features_labels_and_embargo_use_registered_timing() -> None:
    panel = load_r21_price_panel(ROOT)
    specs = load_and_validate_r21_specs(ROOT)
    dataset = build_r21_feature_dataset(panel)
    _, d01_records = build_r21_d01_targets(panel, specs["R21D01"], dataset)
    folds = development_folds(panel.open.index)
...
def test_r21_models_fit_classification_labels_without_future_data() -> None:
    specs = load_and_validate_r21_specs(ROOT)
    ...
    assert set(fitted) == {"R21M01", "R21M02"}
...
def test_r21_frame_hash_binds_session_identity_and_both_label_horizons() -> None:
    ...
    original = _r21_frame_hash(frame, list(MODEL_FEATURES))
    changed = frame.copy()
    changed.loc[0, "m02_label_end_position"] = 22

    assert _r21_frame_hash(frame, list(MODEL_FEATURES)) == original
    assert _r21_frame_hash(changed, list(MODEL_FEATURES)) != original


def test_r21_usd_pressure_latches_at_frozen_tail_boundary_until_trend_recovery() -> None:
...
def test_r21_pressure_sessions_reuse_the_fixed_fast_recovery_signal() -> None:
    dataset = build_r21_feature_dataset(load_r21_price_panel(ROOT))
    expected = (
        ~dataset["risk_on"]
        & (dataset["qqq_trend_gap_50"] > 0.0)
        & (dataset["tqqq_momentum_10"] >= RECOVERY_TQQQ_MOMENTUM_MIN)
        & (dataset["tech_breadth_count_100"] >= RECOVERY_BREADTH_MIN_COUNT)
    )
    pressure = dataset["usd_pressure_active"]

    assert dataset["recovery_boost"].equals(expected)
    assert (pressure & expected).any()
    assert (pressure & ~expected).any()


def test_r21_d01_uses_scheduled_usd100_and_immediate_stress_recovery() -> None:
    ...
    targets, records = build_r21_d01_targets(panel, specs["R21D01"], dataset)
...
def test_r21_m01_authorizes_only_tqqq_override_and_m02_exits_to_qqq(monkeypatch) -> None:
    ...
    def fake_probability(model, current):
        column = "m01_probability" if model.candidate_id == "R21M01" else "m02_probability"
        return float(current.iloc[0][column])

    monkeypatch.setattr(r21, "_fit_route_models", fake_fit)
    monkeypatch.setattr(r21, "_predict_probability", fake_probability)
    ...
    m01 = result.targets["R21M01"]
    m02 = result.targets["R21M02"]
    ...
    assert result.targets["R21F01"].equals(m01)
    assert all(
    ...
    m01_predictions = [
        row["probability"] for row in result.prediction_records if row["candidate_id"] == "R21M01"
    ]
    f01_predictions = [
        row["probability"] for row in result.prediction_records if row["candidate_id"] == "R21F01"
    ]
    assert f01_predictions == m01_predictions


def test_r21_ml_routes_inherit_immediate_stress_recovery(monkeypatch) -> None:
    ...
    monkeypatch.setattr(r21, "_fit_route_models", fake_fit)
    monkeypatch.setattr(r21, "_predict_probability", lambda model, current: 0.90)
...
def test_r21_fold_evaluation_uses_shared_terminal_metric_views() -> None:
    ...
    result = _evaluate_folds(opens, folds, {"R21TEST": targets})["R21TEST"][0]
