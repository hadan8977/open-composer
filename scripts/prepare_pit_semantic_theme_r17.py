...
ITER_ID = "mom_pit_semantic_theme_r17"
...
SOURCE_CARD_PATH = Path("reports/harness/source_cards/us_pit_semantic_theme_r17.jsonl")
SOURCE_SOURCE_CARD_PATH = Path("reports/harness/source_cards/us_pit_semantic_theme_r15.jsonl")
PROMPT_PATH = Path("prompts/pit_semantic_theme_r17_factor_v1.txt")
SPEC_PATHS = {
    candidate_id: Path(f"strategy_specs/drafts/us_pit_semantic_theme_r17_{suffix}.yaml")
...
R17_NEW_SOURCE_CLAIM = (
    "A 2026 paper argues that predictable closing rebalance demand in Korean leveraged ETFs "
    "allowed anticipatory trading to amplify close volatility and transfer value from holders; "
    "the proposed mitigation changes the reference-price mechanism rather than merely spreading "
    "the rebalance through the session."
)
R17_NEW_SOURCE_URL = "https://arxiv.org/abs/2608.03703"
R17_NEW_SOURCE_DOCUMENT_SHA256 = "6d2e828f8702b0ad573768372841400b0936552af10d6ed365202a937b13fb11"
...
def prepare(root: Path) -> None:
    ...
    if quality.get("status") != "ok" or quality.get("research_eligible") is not True:
        raise ValueError("R17 preparation requires a passing price-adjustment quality report")
    if snapshot.get("request_count") != 72:
        raise ValueError("R17 snapshot must bind 18 symbols x four adjustment modes")
...
def finalize(root: Path) -> None:
    ...
    for name in (
        ...
        if not (iteration / name).is_file():
            raise ValueError(f"R17 finalization prerequisite is missing: {name}")
    assessment = _load_json(iteration / "knowledge-assessment.json")
    if assessment.get("status") != "ok":
        raise ValueError("R17 knowledge assessment must pass before finalization")
...
def _write_capability_reviews(root: Path) -> None:
    ...
    for candidate_id in ("R17D02", "R17L01", "R17C01", "R17P01"):
        ...
        payload = {
            ...
            "capabilities": [
                {
                    ...
                    "paper_ready": True,
                    "notes": (
                        "Daily bars are supported; R17 historical research binds an immutable "
            ...
            "blockers": [
                "new R17 forward epoch has no counted live semantic sessions",
    ...
    *,
    ...
    return {
        ...
        "params": {
            ...
            "resolver": "pit_semantic_theme_r17_panel_feature_v1",
        },
    }
...
def _write_specs(root: Path) -> dict[str, Any]:
    source_specs = {
        candidate_id.replace("R17", "R15", 1): yaml.safe_load(
            (
                root / str(path).replace("us_pit_semantic_theme_r17", "us_pit_semantic_theme_r15")
    ...
    d01["portfolio"].update(
        {
            ...
            "selected_route_label": "pit_semantic_theme_r17:D01:stress_recovery_boost_v1",
        }
    )
    d01["factors"] = {
        "qqq_momentum_120": {
    ...
    d01["notes"].update(
        {
            ...
            "method": "qqq_tqqq_stress_recovery_soxl100_state_v1",
    ...
    m01["portfolio"].update(
        {
            ...
            "gross_exposure_limit": 1.0,
            "max_symbol_weight": 1.0,
            "selected_route_label": "pit_semantic_theme_r17:M01:leadership_soxl100_logit_v1",
    ...
    m02["name"] = "us_pit_semantic_theme_r17_m02"
    m02["description"] = (
    ...
    m02["portfolio"]["selected_route_label"] = "pit_semantic_theme_r17:M02:tail_exit_lgbm_v1"
    ...
    m02["notes"].update(
        {
            ...
            "fallback_candidate_id": "R17M01_then_R17D01",
            "low_confidence_action": "preserve_R17M01_target",
            "factor_library_ids": [
    ...
    m02["research_design"]["parameter_space"]["candidate_id"] = ["R17M02"]
    generated["R17M02"] = m02

    f01 = json.loads(json.dumps(m01))
    f01["name"] = "us_pit_semantic_theme_r17_f01"
    f01["description"] = "Exact no-semantic-input identity control for R17M01."
    f01["portfolio"]["selected_route_label"] = "pit_semantic_theme_r17:F01:exact_m01_fallback"
    f01["notes"].update(
        {
            "candidate_id": "R17F01",
            "candidate_role": "missing_modality_exact_m01_fallback",
            "method": "exact_r17m01_without_semantic_inputs_v1",
            "fallback_candidate_id": "R17M01_then_R17D01",
        }
    )
    f01["research_design"]["parameter_space"]["candidate_id"] = ["R17F01"]
    generated["R17F01"] = f01

    semantic_factor_names = {
        "R17C01": ("theme_confidence", "theme_direction"),
        "R17P01": ("shuffled_theme_direction",),
    }
    for candidate_id in ("R17C01", "R17P01"):
    ...
    generated["R17P01"]["notes"]["placebo_seed"] = 15108
    ...
    for candidate_id, raw in generated.items():
        ...
        if raw.get("execution_policy"):
            raw["execution_policy"]["policy_id"] = "r17_protected_opg_v1"
    ...
    retained.append(
        {
            ...
            "verified_at": "2026-08-05T19:20:27Z",
            "verification_method": "fresh_arxiv_official_api_metadata_and_abstract",
            "claim_id": "r17_leveraged_etf_rebalance_microstructure_risk",
            "claim": R17_NEW_SOURCE_CLAIM,
            "source_url": R17_NEW_SOURCE_URL,
            "source_type": "paper",
            "accessed_at": "2026-08-05",
            "document_updated_at": "2026-08-04",
            "retrieved_document_sha256": R17_NEW_SOURCE_DOCUMENT_SHA256,
            "applies_to": [
                "leveraged_etf",
            ...
            "impact_on_spec": (
                "Treat leveraged-ETF rebalance mechanics as execution and TCA risk; preserve "
                "cost, capacity, open-gap, and matched-fill gates without treating this paper as "
                "evidence for the recovery signal."
            ),
            "limitations": (
                "The study concerns 2026 Korean leveraged ETFs and closing-rebalance mechanics; "
                "it is not US/TQQQ Alpha evidence and does not validate R17 timing or paper fills."
            ),
        }
    )
    target.write_text(
...
def _customize_contract(
    ...
    if name == "data-contract.json":
        ...
        scope.update(
            {
                ...
                "allowed_candidate_ids": ["R17D01", "R17M01", "R17M02", "R17F01"],
                "limitations": [
        ...
        payload["single_hypothesis"] = (
            "after R16 showed that unconditional SQQQ in the slow stress state is destructive, "
            "restore R15's QQQ100 stress target and temporarily recover to TQQQ100 only when "
            "QQQ is above SMA50, TQQQ ten-session momentum is at least eight percent, and at "
            "least four of five technology canaries are above SMA100; all other mechanics stay "
            "frozen and semantic themes remain forward-only and zero-capital"
        )
        payload["semantic_features"]["prompt_path"] = PROMPT_PATH.as_posix()
        payload["capital"]["fallbacks"]["R17M02"] = "R17M01_then_R17D01"
        payload["deterministic_route"] = {
            "candidate_id": "R17D01",
            "route_label": "pit_semantic_theme_r17:D01:stress_recovery_boost_v1",
        ...
        payload["quant_features"].update(
            {
                "candidate_ids": ["R17M01", "R17M02", "R17C01", "R17F01", "R17P01"],
                "families": [
        ...
        payload["pbo"]["selection_candidate_ids"] = ["R17D01", "R17M01", "R17M02"]
        payload["model_gates"] = {
            "R17M01": (
            ...
            "R17F01": "target and prediction identity with M01 for every matched session",
        }
        ...
        payload["semantic_gates"]["stock_budget_during_R17"] = 0.0
        ...
        payload.update(
            {
                "prior_effective_trial_count": 8078,
                "effective_trial_count": 8086,
                "prior_trial_evidence": {
                    "path": (
                        "reports/research/iterations/mom_pit_semantic_theme_r16/"
                        "historical-evaluation/evaluation-report.json"
                    ),
                    "status": "valid_clean_data_stop_price_paths_R16",
                },
            }
        )
    elif name == "universe-contract.json":
        payload["contract_id"] = "r17_dynamic_universe_v1"
                ...
                row.update(
                    {
                        ...
                        "candidates": ["R17D01", "R17M01", "R17C01", "R17F01", "R17P01"],
                ...
                row.update(
                    {
                        ...
                        "candidates": ["R17D01", "R17M02"],
                        "method": "deterministic_trend_off_plus_fold_local_tail_survival",
                        "fallback": "R17M01_then_R17D01",
                    }
                )
            if row["role"] == "regime_meta_gate":
                row.update(
                    {
                        "candidates": ["R17D01", "R17M01", "R17M02"],
                        "method": "full_gross_QQQ_TQQQ_recovery_SOXL_state_with_rare_QQQ_tail_exit",
                        "fallback": "R17D01",
                    }
                )
        for row in payload["required_ablations"]:
            if row["candidate"] == "R17M01":
                row["ablation"] = "quant_only_logistic_semiconductor_SOXL100"
            elif row["candidate"] == "R17M02":
        ...
        payload["scout_network_status"] = {
            ...
            "action": (
                "retrieved the official arXiv API record for a new leveraged-ETF rebalance "
                "microstructure paper; its Korean closing-auction result is execution-risk "
                "context, not US/TQQQ Alpha evidence"
            ),
        }
...
def _model_reuse_round_decision(root: Path) -> dict[str, Any]:
    return {
        ...
        "reason": (
            ...
            "drawdown while using QQQ for 272 of 1008 development sessions. R16 then replaced "
            "those sessions with SQQQ and failed at negative CAGR. R17 restores QQQ and changes "
            "only a fixed stress-recovery route; M01 and M02 features, labels, folds, seeds, "
            "cadence, and thresholds remain fixed and are refit fold-locally without loading "
            "serialized prior estimators"
        ),
        "source_iteration": SOURCE_ITER,
        "source_status": "stop_price_paths",
        "negative_evidence_iteration": "mom_pit_semantic_theme_r16",
...
def _write_external_brief(root: Path, specs: dict[str, Any], generated_at: str) -> None:
    ...
    payload.update(
        {
            "iter_id": ITER_ID,
            "strategy_name": specs["R17D01"].name,
            "objective": (
                "Test one stress-recovery hypothesis against the restored R15 mechanics: QQQ in "
                "confirmed slow stress, temporary TQQQ only when the fixed fast-trend, momentum, "
                "and breadth conditions all pass, and SOXL100 only during confirmed semiconductor "
                "leadership. Fold-local models, QQQ tail exits, and forward-only zero-capital "
                "semantic themes otherwise remain unchanged."
            ),
            "source_spec_path": SPEC_PATHS["R17D01"].as_posix(),
            "spec_hash": strategy_content_hash(specs["R17D01"]),
            "current_source_card_paths": [SOURCE_CARD_PATH.as_posix()],
            "hypothesis_links": ["R17-H1"],
            "generated_at": generated_at,
        }
    )
    revisions = {row["candidate_id"]: row for row in payload["candidate_matrix_revisions"]}
    revisions["R17D01"]["revision"] = (
        "restore R15 QQQ100 after R16's SQQQ failure and add only the fixed temporary TQQQ "
        "stress-recovery state"
    )
    revisions["R17M01"]["revision"] = (
        "refit the unchanged fold-local SOXL100-vs-TQQQ Logistic contract; low confidence is "
        "exact D01 including the QQQ/TQQQ stress-recovery route"
    )
    revisions["R17M02"]["revision"] = (
        "fold-local LightGBM path-survival diagnostic can only make high-confidence tail exits"
    )
    new_source = {
        "core_claim": R17_NEW_SOURCE_CLAIM,
        "credibility": "freshly retrieved official arXiv API metadata and abstract",
        "project_applicability": (
            "Retain leveraged-ETF transaction-cost, capacity, and execution-stress controls; the "
            "paper does not supply or tune the R17 recovery signal."
        ),
        "published_or_updated_at": "2026-08-04",
        "reflection": (
            "A Korean closing-rebalance study does not establish US/TQQQ Alpha, R17 stress "
            "timing, opening-fill quality, or paper readiness."
        ),
        "source_type": "paper",
        "url": R17_NEW_SOURCE_URL,
    }
    payload["sources"].append(new_source)
    payload["source_evidence_bindings"].append(
        {
            "brief_claim_fingerprint": claim_fingerprint(R17_NEW_SOURCE_CLAIM),
            "canonical_url": R17_NEW_SOURCE_URL,
            "claim_fingerprint": claim_fingerprint(R17_NEW_SOURCE_CLAIM),
            "source_card_claim_id": "r17_leveraged_etf_rebalance_microstructure_risk",
        }
    )
    payload["topic_coverage"].append("leveraged_ETF_rebalance_microstructure_risk")
...
def _write_markdown(root: Path) -> None:
    ...
    (iteration / "hypotheses.md").write_text(
        "\n".join(
            [
                "# Hypotheses: mom_pit_semantic_theme_r17",
                "",
                "## R17-H1",
                "",
                (
                    "- Hypothesis: R15's 272 QQQ stress sessions protected drawdown but missed "
                    "fast rebounds, while R16 proved unconditional SQQQ was destructive. Restore "
                    "QQQ and recover temporarily to TQQQ only when QQQ is above SMA50, TQQQ "
                    "ten-session momentum is at least 8%, and at least four of five technology "
                    "canaries are above SMA100; all other mechanics stay frozen."
                ),
                (
                    "- Failure mode: rebound whipsaw and extra turnover push drawdown below -65%, "
                    "D01 or M01 still misses 45% net CAGR, fewer than three folds beat QQQ, or "
                    "semantic packets cannot prove changing membership."
                ),
                (
                ...
                (
                    "- Stop/Pivot criterion: do not tune any R17 threshold after results. Open the "
    ...
    (iteration / "search-space.md").write_text(
        "\n".join(
            [
                "# Search Space: mom_pit_semantic_theme_r17",
                "",
                (
    ...
    (iteration / "decision-record.md").write_text(
        "\n".join(
            [
                "# Decision Record: mom_pit_semantic_theme_r17",
                "",
                "## Preregistration",
                "",
                ...
                (
                    ...
                    "272 of 1008 development sessions. R16's unconditional SQQQ replacement then "
                    "produced negative CAGR, so R17 restores QQQ and tests one short-cycle rebound "
                    "mechanism without changing models, labels, scheduling, costs, or folds."
                ),
                (
    ...
    (iteration / "external-brief.md").write_text(
        "\n".join(
            [
                "# External Brief: mom_pit_semantic_theme_r17",
                "",
                (
                    "R17 reuses the verified evidence assembled through R16 on time-varying "
                    "product "
                    ...
                    "execution. The new empirical question is narrow: after restoring QQQ from "
                    "R16's failed SQQQ route, does a fixed short-cycle TQQQ recovery condition "
                    "repair R15's return shortfall while preserving normal TQQQ, SOXL100 "
                    "leadership, and rare QQQ tail exits? AI infrastructure and robotics remain "
                    "examples of current "
                    "event-defined "
...
def _candidate_manifest(root: Path, specs: dict[str, Any]) -> dict[str, Any]:
    ...
    contract_names = {
        "data": ("r17_pit_semantic_theme_data_v1", "data-contract.json"),
        "features": ("r17_pit_semantic_theme_features_v1", "feature-contract.json"),
        "labels": ("r17_labels_v1", "label-contract.json"),
        "validation": ("r17_validation_v1", "validation-contract.json"),
        "costs": ("r17_costs_v1", "cost-contract.json"),
        "benchmarks": ("r17_benchmark_family_v1", "benchmark-contract.json"),
    }
    source["contracts"] = {
        group: {
            contract_id: {
    ...
    by_id["R17D01"].update(
        {
            ...
            "method": "qqq_tqqq_stress_recovery_semiconductor_soxl100_state_v1",
            "ablation": "deterministic_fast_recovery_high_beta",
    ...
    by_id["R17P01"]["placebo_seed"] = 15108
    return source
...
def _write_feasibility_and_search(root: Path, *, include_knowledge_assessment: bool) -> None:
    ...
    payload.update(
        {
            "report_type": "pit_semantic_theme_r17_data_feasibility",
            "iter_id": ITER_ID,
            "generated_at": datetime.now(UTC).isoformat(),
    ...
    search.update(
        {
            "iter_id": ITER_ID,
            "objective": (
                "Test a fixed temporary TQQQ recovery state inside the restored full-gross "
                "QQQ/TQQQ/SOXL route, the unchanged fold-local Logistic authorization model, a "
    ...
    d01_spec = load_strategy_spec(root / SPEC_PATHS["R17D01"])
    search["source_spec_path"] = SPEC_PATHS["R17D01"].as_posix()
...
def _replace_text(value: str) -> str:
    return value.replace("R15", "R17").replace("r15", "r17")
