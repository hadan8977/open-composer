...
SOURCE_ITER = "mom_pit_semantic_theme_r21"
ITER_ID = "mom_pit_semantic_theme_r22"
...
SOURCE_CARD_PATH = Path("reports/harness/source_cards/us_pit_semantic_theme_r22.jsonl")
SOURCE_SOURCE_CARD_PATH = Path("reports/harness/source_cards/us_pit_semantic_theme_r21.jsonl")
PROMPT_PATH = Path("prompts/pit_semantic_theme_r22_factor_v1.txt")
SPEC_PATHS = {
    candidate_id: Path(f"strategy_specs/drafts/us_pit_semantic_theme_r22_{suffix}.yaml")
...
R22_TREND_HORIZON_SOURCE_CLAIM = (
    "A 2025 trend-premia study reports that, in its asset-level Bayesian allocation framework, "
    "the medium-term 125-day layer adds little incremental performance once short- and "
    "long-term components are present; removing that layer improves reported Sharpe and "
    "drawdown efficiency while maintaining benchmark correlation."
)
R22_TREND_HORIZON_SOURCE_URL = "https://arxiv.org/abs/2510.23150"
R22_TREND_HORIZON_DOCUMENT_SHA256 = (
    "02130f60884e440eb7e68d40cb3f466e245ef52afcc4ba3ee671b35df2e68161"
)
...
def prepare(root: Path) -> None:
    ...
    if quality.get("status") != "ok" or quality.get("research_eligible") is not True:
        raise ValueError("R22 preparation requires a passing price-adjustment quality report")
    if snapshot.get("request_count") != 72:
        raise ValueError("R22 snapshot must bind 18 symbols x four adjustment modes")
    ...
    for name in CONTRACT_FILES:
        payload = _replace_r21(_load_json(base / SOURCE_DIR / name))
...
def finalize(root: Path) -> None:
    ...
    for name in (
        ...
        if not (iteration / name).is_file():
            raise ValueError(f"R22 finalization prerequisite is missing: {name}")
    assessment = _load_json(iteration / "knowledge-assessment.json")
    if assessment.get("status") != "ok":
        raise ValueError("R22 knowledge assessment must pass before finalization")
...
def _write_capability_reviews(root: Path) -> None:
    ...
    for candidate_id in ("R22D02", "R22L01", "R22C01", "R22P01"):
        ...
        payload = {
            ...
            "capabilities": [
                {
                    ...
                    "paper_ready": True,
                    "notes": (
                        "Daily bars are supported; R22 historical research binds an immutable "
            ...
            "blockers": [
                "new R22 forward epoch has no counted live semantic sessions",
    ...
    *,
    ...
    return {
        ...
        "params": {
            ...
            "resolver": "pit_semantic_theme_r22_panel_feature_v1",
        },
    }
...
def _write_specs(root: Path) -> dict[str, Any]:
    source_specs = {
        candidate_id.replace("R22", "R21", 1): yaml.safe_load(
            (
                root / str(path).replace("us_pit_semantic_theme_r22", "us_pit_semantic_theme_r21")
    ...
    for candidate_id in SPEC_PATHS:
        source_id = candidate_id.replace("R22", "R21", 1)
        raw = _replace_r21(source_specs[source_id])
        generated[candidate_id] = raw

    d01 = generated["R22D01"]
    d01["description"] = (
        "Full-gross high-beta state: hold QQQ in confirmed Nasdaq stress, recover temporarily "
        "to TQQQ on the frozen three-part rebound signal, and otherwise hold USD at full weight. "
        "A USD 20-session drawdown latch defaults to QQQ but permits TQQQ when the two frozen "
        "fast price confirmations pass without waiting for the slower breadth confirmation."
    )
    ...
    d01["portfolio"].update(
        {
            ...
            "selected_route_label": "pit_semantic_theme_r22:D01:usd_pressure_dual_fast_recovery_v1",
        }
    )
    d01["factors"] = {
        "qqq_momentum_120": {
    ...
    m01["description"] = (
        ...
        "exact D01, including its pressure-specific dual fast-price recovery state."
    )
    ...
    m01["portfolio"].update(
        {
            ...
            "gross_exposure_limit": 1.0,
            "max_symbol_weight": 1.0,
            "selected_route_label": "pit_semantic_theme_r22:M01:tqqq_override_logit_v1",
    ...
    m02["name"] = "us_pit_semantic_theme_r22_m02"
    m02["description"] = (
    ...
    m02["portfolio"]["selected_route_label"] = "pit_semantic_theme_r22:M02:tail_exit_lgbm_v1"
    ...
    m02["notes"].update(
        {
            ...
            "fallback_candidate_id": "R22M01_then_R22D01",
            "low_confidence_action": "preserve_R22M01_target",
            "factor_library_ids": [
                "drawdown_guard_20_60",
    ...
    m02["research_design"]["parameter_space"]["candidate_id"] = ["R22M02"]
    generated["R22M02"] = m02

    f01 = json.loads(json.dumps(m01))
    f01["name"] = "us_pit_semantic_theme_r22_f01"
    f01["description"] = "Exact no-semantic-input identity control for R22M01."
    f01["portfolio"]["selected_route_label"] = "pit_semantic_theme_r22:F01:exact_m01_fallback"
    f01["notes"].update(
        {
            "candidate_id": "R22F01",
            "candidate_role": "missing_modality_exact_m01_fallback",
            "method": "exact_r22m01_without_semantic_inputs_v1",
            "fallback_candidate_id": "R22M01_then_R22D01",
        }
    )
    f01["research_design"]["parameter_space"]["candidate_id"] = ["R22F01"]
    generated["R22F01"] = f01

    semantic_factor_names = {
        "R22C01": ("theme_confidence", "theme_direction"),
        "R22P01": ("shuffled_theme_direction",),
    }
    for candidate_id in ("R22C01", "R22P01"):
    ...
    generated["R22P01"]["notes"]["placebo_seed"] = 15108
    ...
    for candidate_id, raw in generated.items():
        ...
        if raw.get("execution_policy"):
            raw["execution_policy"]["policy_id"] = "r22_protected_opg_v1"
...
def _write_prompt(root: Path) -> None:
    source = root / "prompts/pit_semantic_theme_r21_factor_v1.txt"
...
def _write_source_cards(root: Path) -> None:
    ...
    for line in source.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        row = json.loads(line)
        retained.append(_replace_r21(row))
    retained.append(
        {
            "iteration_id": ITER_ID,
            "verification_status": "source_verified",
            "verified_at": "2026-08-05T23:20:54Z",
            "verification_method": "fresh_arxiv_official_api_metadata_and_abstract",
            "claim_id": "r22_trend_horizon_redundancy",
            "claim": R22_TREND_HORIZON_SOURCE_CLAIM,
            "source_url": R22_TREND_HORIZON_SOURCE_URL,
            "source_type": "paper",
            "accessed_at": "2026-08-05",
            "document_updated_at": "2025-10-28",
            "retrieved_document_sha256": R22_TREND_HORIZON_DOCUMENT_SHA256,
            "applies_to": ["trend_horizon", "recovery_confirmation", "R22D01"],
            "impact_on_spec": (
                "Supports a preregistered test of whether the slower breadth confirmation is "
                "redundant during a separately identified USD-pressure state; it does not "
                "change either frozen fast threshold or ordinary stress recovery."
            ),
            "limitations": (
                "The paper studies optimized cross-asset trend-horizon allocations, not market "
                "breadth, leveraged ETFs, USD pressure, or this strategy. It does not validate "
                "R22 returns, costs, thresholds, Alpha, or Paper readiness."
            ),
        }
    )
    target.write_text(
...
def _customize_contract(
    ...
    if name == "data-contract.json":
        ...
        scope.update(
            {
                ...
                "allowed_candidate_ids": ["R22D01", "R22M01", "R22M02", "R22F01"],
                "limitations": [
        ...
        payload["single_hypothesis"] = (
            "preserve the complete R21 USD pressure latch and every model contract, but in an "
            "active USD pressure state require only the frozen QQQ trend and TQQQ short-momentum "
            "confirmations rather than the slow 100-session breadth confirmation; ordinary "
            "stress recovery and all thresholds, schedules, folds, costs, and semantic roles "
            "remain frozen"
        )
        payload["semantic_features"]["prompt_path"] = PROMPT_PATH.as_posix()
        payload["capital"]["fallbacks"]["R22M02"] = "R22M01_then_R22D01"
        payload["deterministic_route"] = {
            "candidate_id": "R22D01",
            "route_label": "pit_semantic_theme_r22:D01:usd_pressure_dual_fast_recovery_v1",
        ...
        payload["quant_features"].update(
            {
                "candidate_ids": ["R22M01", "R22M02", "R22C01", "R22F01", "R22P01"],
                "families": [
        ...
        payload["pbo"]["selection_candidate_ids"] = ["R22D01", "R22M01", "R22M02"]
        payload["model_gates"] = {
            "R22M01": (
            ...
            "R22F01": "target and prediction identity with M01 for every matched session",
        }
        ...
        payload["semantic_gates"]["stock_budget_during_R22"] = 0.0
        ...
        payload.update(
            {
                "prior_effective_trial_count": 8118,
                "effective_trial_count": 8126,
                "prior_trial_evidence": {
                    "path": (
                        "reports/research/iterations/mom_pit_semantic_theme_r21/"
                        "historical-evaluation/evaluation-report.json"
                    ),
                    "status": "valid_clean_data_stop_price_paths_R21",
                },
                "adaptive_selection_disclosure": (
                    "USD was the exposed development window's ex-post best benchmark symbol; "
                    "R21 pressure-spell outcomes are also exposed; R22 development evidence is "
        ...
        payload["contract_id"] = "r22_dynamic_universe_v1"
                ...
                row.update(
                    {
                        ...
                        "candidates": ["R22D01", "R22M01", "R22C01", "R22F01", "R22P01"],
        ...
        payload["scout_network_status"] = {
            ...
            "action": (
                "reuse the verified leveraged-ETF trend, crash-brake, and July 2026 temporal "
                "tail-protection claims; add the trend-horizon redundancy study; and retain "
                "finalized R21 recovery-condition negative evidence"
            ),
        }
...
def _model_reuse_round_decision(root: Path) -> dict[str, Any]:
    return {
        ...
        "reason": (
            "R21D01 reached 43.85% net CAGR with -66.62% maximum drawdown, but missed the "
            "CAGR, drawdown, up-capture, and DSR gates. Across 348 USD-pressure sessions, both "
            "fast price confirmations passed on 49 sessions while the complete recovery with "
            "100-session breadth passed on only 17. R22 changes only the pressure-state breadth "
            "requirement and preserves all model families, schedules, costs, folds, and numeric "
            "thresholds. Both models refit fold-locally without prior estimators"
        ),
        "source_iteration": SOURCE_ITER,
        "source_status": "stop_price_paths",
        "negative_evidence_iteration": "mom_pit_semantic_theme_r21",
...
def _write_external_brief(root: Path, specs: dict[str, Any], generated_at: str) -> None:
    payload = _replace_r21(_load_json(root / SOURCE_DIR / "external-brief.json"))
    payload.update(
        {
            "iter_id": ITER_ID,
            "strategy_name": specs["R22D01"].name,
            "objective": (
                "Preserve the complete R21 price and model route, but while the USD pressure "
                "latch is active require only the frozen QQQ trend and TQQQ short-momentum "
                "confirmations for TQQQ recovery. A failed fast condition returns immediately "
                "to QQQ. Ordinary stress recovery, model families, thresholds, folds, costs, "
                "and zero-capital semantic roles remain unchanged."
            ),
            "source_spec_path": SPEC_PATHS["R22D01"].as_posix(),
            "spec_hash": strategy_content_hash(specs["R22D01"]),
            "current_source_card_paths": [SOURCE_CARD_PATH.as_posix()],
            "hypothesis_links": ["R22-H1"],
            "generated_at": generated_at,
        }
    )
    revisions = {row["candidate_id"]: row for row in payload["candidate_matrix_revisions"]}
    revisions["R22D01"]["revision"] = (
        "preserve R21 except omit slow breadth from USD-pressure recovery"
    )
    revisions["R22M01"]["revision"] = (
        "preserve R21 Logistic TQQQ override and inherit pressure-state dual fast recovery"
    )
    revisions["R22M02"]["revision"] = (
        "preserve R21 LightGBM path-survival diagnostic on the newly scheduled D01 route"
    )
    payload["sources"].append(
        {
            "core_claim": R22_TREND_HORIZON_SOURCE_CLAIM,
            "credibility": "freshly retrieved arXiv official API metadata and abstract",
            "project_applicability": (
                "Motivates testing whether a slower confirmation layer is redundant when short "
                "and long state information already coexist; it does not select the R22 rule."
            ),
            "published_or_updated_at": "2025-10-28",
            "reflection": (
                "The source studies a different optimized cross-asset framework and does not "
                "establish breadth redundancy, R22 Alpha, or Paper readiness."
            ),
            "source_type": "paper",
            "url": R22_TREND_HORIZON_SOURCE_URL,
        }
    )
    payload["source_evidence_bindings"].append(
        {
            "brief_claim_fingerprint": claim_fingerprint(R22_TREND_HORIZON_SOURCE_CLAIM),
            "canonical_url": R22_TREND_HORIZON_SOURCE_URL,
            "claim_fingerprint": claim_fingerprint(R22_TREND_HORIZON_SOURCE_CLAIM),
            "source_card_claim_id": "r22_trend_horizon_redundancy",
        }
    )
    payload["topic_coverage"].append("trend_horizon_redundancy")
...
def _write_markdown(root: Path) -> None:
    ...
    (iteration / "hypotheses.md").write_text(
        "\n".join(
            [
                "# Hypotheses: mom_pit_semantic_theme_r22",
                "",
                "## R22-H1",
                "",
                (
                    "- Hypothesis: R21D01 reached 43.85% net CAGR but recovered to TQQQ on only "
                    "17 of 348 USD-pressure sessions. The two frozen fast price confirmations "
                    "were jointly positive on 49 sessions, while the slow 100-session breadth "
                    "confirmation was binding. Keep the latch and both fast thresholds, omit "
                    "breadth only during USD pressure, and return immediately to QQQ when either "
                    "fast condition fails."
                ),
                (
                ...
                (
                    "- Stop/Pivot criterion: do not tune any R22 threshold after results. Open the "
                ...
                (
                    "- Adaptiveness disclosure: USD, all R21 recovery-condition outcomes, and "
                    "the failing folds are exposed. R22 reuses every numeric threshold without "
                    "search and removes one exposed condition in one state. A development pass "
                    "remains adaptive and cannot by itself authorize Alpha, Tier 0 capital, or "
                    "Paper."
                ),
                "",
            ]
    ...
    (iteration / "search-space.md").write_text(
        "\n".join(
            [
                "# Search Space: mom_pit_semantic_theme_r22",
                "",
                (
    ...
    (iteration / "decision-record.md").write_text(
        "\n".join(
            [
                "# Decision Record: mom_pit_semantic_theme_r22",
                "",
                "## Preregistration",
                "",
                ...
                (
                    "- Reason: R21D01 reached 43.85% net CAGR and -66.62% MDD but failed CAGR, "
                    "drawdown, up-capture, and DSR. During 348 USD-pressure sessions, both fast "
                    "price confirmations passed on 49 sessions while the complete recovery with "
                    "slow breadth passed on only 17. R22 removes breadth only from USD-pressure "
                    "recovery and preserves the latch, numeric thresholds, ordinary stress "
                    "recovery, models, scheduling, costs, folds, and holdout."
                ),
                (
    ...
    (iteration / "external-brief.md").write_text(
        "\n".join(
            [
                "# External Brief: mom_pit_semantic_theme_r22",
                "",
                (
                    "R22 reuses the verified evidence assembled through R21 on time-varying "
                    "product "
                    ...
                    "drawdown latch retain crash protection when pressure recovery uses two "
                    "fast price confirmations without waiting for slow breadth? The latch and "
                    "all numeric thresholds are reused unchanged; no sweep is allowed. USD, "
                    "R21 pressure outcomes, and the "
...
def _candidate_manifest(root: Path, specs: dict[str, Any]) -> dict[str, Any]:
    source = _replace_r21(_load_json(root / SOURCE_DIR / "candidate-manifest.json"))
    ...
    contract_names = {
        "data": ("r22_pit_semantic_theme_data_v1", "data-contract.json"),
        "features": ("r22_pit_semantic_theme_features_v1", "feature-contract.json"),
        "labels": ("r22_labels_v1", "label-contract.json"),
        "validation": ("r22_validation_v1", "validation-contract.json"),
        "costs": ("r22_costs_v1", "cost-contract.json"),
        "benchmarks": ("r22_benchmark_family_v1", "benchmark-contract.json"),
    }
    source["contracts"] = {
        group: {
            contract_id: {
    ...
    by_id["R22D01"].update(
        {
            "role": "deterministic_usd_pressure_dual_fast_recovery",
            "method": "usd_pressure_dual_fast_price_recovery_v1",
            "ablation": "R21D01_pressure_requires_slow_breadth",
            "fallback": "R21D01",
        }
    )
    by_id["R22M01"].update(
    ...
    by_id["R22P01"]["placebo_seed"] = 15108
    return source
...
def _write_feasibility_and_search(root: Path, *, include_knowledge_assessment: bool) -> None:
    ...
    payload = _replace_r21(_load_json(root / SOURCE_DIR / "data-feasibility.json"))
    payload.update(
        {
            "report_type": "pit_semantic_theme_r22_data_feasibility",
            "iter_id": ITER_ID,
            "generated_at": datetime.now(UTC).isoformat(),
    ...
    payload["data_scope"] = {
        ...
        "adaptive_price_selection": (
            "USD, R21's weak folds, and all R21 pressure-spell outcomes are exposed; the latch "
    ...
    search = _replace_r21(_load_json(root / SOURCE_DIR / "search-space.json"))
    search.update(
        {
            "iter_id": ITER_ID,
            "objective": (
                "Preserve the complete R21 route and models, but omit the slow breadth "
                "confirmation from USD-pressure recovery while retaining both frozen fast price "
                "confirmations; test whether rebound participation returns without losing "
                "persistent-selloff protection; semantic themes remain forward-only."
            ),
    ...
    d01_spec = load_strategy_spec(root / SPEC_PATHS["R22D01"])
    search["source_spec_path"] = SPEC_PATHS["R22D01"].as_posix()
    ...
    trained["parameters"].update(
        {
            ...
            "embargo_sessions": 21,
            "purge_sessions": 21,
            "label_horizon_sessions": {"R22M01": 20, "R22M02": 10},
        }
    )
    ...
    search["adaptive_selection_disclosure"] = (
        "USD, R21 recovery-condition outcomes, and fold attribution are exposed; the latch and "
        "all numeric recovery thresholds are inherited rather than searched, and only a "
        "separately locked one-time transfer-holdout evaluation may provide independent "
        "selection evidence"
    )
...
def _replace_r21(value: Any) -> Any:
    if isinstance(value, dict):
        return {_replace_text(str(key)): _replace_r21(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_replace_r21(item) for item in value]
...
def _replace_text(value: str) -> str:
    return value.replace("R21", "R22").replace("r21", "r22")
