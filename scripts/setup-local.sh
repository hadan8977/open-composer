...
if [[ $DRY_RUN -eq 1 ]]; then
  ...
  if [[ -f reports/dashboard/catalog.json ]]; then
    s_strat=$(python3 -c "import json; print(json.load(open('reports/dashboard/catalog.json'))['summary'].get('strategy_count', 0))" 2>/dev/null || echo "?")
    s_ver=$(python3 -c "import json; print(json.load(open('reports/dashboard/catalog.json'))['summary'].get('version_count', 0))" 2>/dev/null || echo "?")
    s_sig=$(python3 -c "import json; print(json.load(open('reports/dashboard/catalog.json'))['summary'].get('signal_count', 0))" 2>/dev/null || echo "?")
...
echo "  - Try a strategy:      uv run oc strategy draft \"...\""
