...
SOURCE_ITER = "mom_pit_semantic_theme_r14"
ITER_ID = "mom_pit_semantic_theme_r15"
...
SOURCE_CARD_PATH = Path("reports/harness/source_cards/us_pit_semantic_theme_r15.jsonl")
SOURCE_SOURCE_CARD_PATH = Path("reports/harness/source_cards/us_pit_semantic_theme_r14.jsonl")
PROMPT_PATH = Path("prompts/pit_semantic_theme_r15_factor_v1.txt")
SPEC_PATHS = {
    candidate_id: Path(f"strategy_specs/drafts/us_pit_semantic_theme_r15_{suffix}.yaml")
...
R15_NEW_SOURCE_CLAIM = (
    "A 2026 working paper applying volatility-robust bubble tests to AI-exposed equities "
    "reports heterogeneous timing, intensity, and duration of exuberance across the Magnificent "
    "Seven and leading semiconductor firms."
)
R15_NEW_SOURCE_URL = "https://arxiv.org/abs/2604.12062"
...
def prepare(root: Path) -> None:
    ...
    if quality.get("status") != "ok" or quality.get("research_eligible") is not True:
        raise ValueError("R15 preparation requires a passing price-adjustment quality report")
    if snapshot.get("request_count") != 72:
        raise ValueError("R15 snapshot must bind 18 symbols x four adjustment modes")
    ...
    for name in CONTRACT_FILES:
        payload = _replace_r14(_load_json(base / SOURCE_DIR / name))
...
def finalize(root: Path) -> None:
    ...
    for name in (
        ...
        if not (iteration / name).is_file():
            raise ValueError(f"R15 finalization prerequisite is missing: {name}")
    assessment = _load_json(iteration / "knowledge-assessment.json")
    if assessment.get("status") != "ok":
        raise ValueError("R15 knowledge assessment must pass before finalization")
...
def _write_capability_reviews(root: Path) -> None:
    ...
    for candidate_id in ("R15D02", "R15L01", "R15C01", "R15P01"):
        ...
        payload = {
            ...
            "capabilities": [
                {
                    ...
                    "paper_ready": True,
                    "notes": (
                        "Daily bars are supported; R15 historical research binds an immutable "
            ...
            "blockers": [
                "new R15 forward epoch has no counted live semantic sessions",
    ...
    *,
    ...
    return {
        ...
        "params": {
            ...
            "resolver": "pit_semantic_theme_r15_panel_feature_v1",
        },
    }
...
def _write_specs(root: Path) -> dict[str, Any]:
    source_specs = {
        candidate_id.replace("R15", "R14", 1): yaml.safe_load(
            (
                root / str(path).replace("us_pit_semantic_theme_r15", "us_pit_semantic_theme_r14")
    ...
    for candidate_id in SPEC_PATHS:
        source_id = candidate_id.replace("R15", "R14", 1)
        raw = _replace_r14(source_specs[source_id])
        generated[candidate_id] = raw

    d01 = generated["R15D01"]
    d01["description"] = (
    ...
    d01["portfolio"].update(
        {
            ...
            "selected_route_label": "pit_semantic_theme_r15:D01:stable_leverage_state_v1",
        }
    )
    d01["factors"] = {
        "qqq_momentum_120": {
    ...
    d01["notes"].update(
        {
            ...
            "method": "weekly_semiconductor_leadership_soxl100_v1",
    ...
    m01 = generated["R15M01"]
    m01["description"] = (
        "Fold-local Logistic classifier that may authorize only the fixed SOXL100 leadership "
        "target; low confidence preserves TQQQ or the deterministic QQQ stress target."
    )
    ...
    m01["portfolio"].update(
        {
            ...
            "gross_exposure_limit": 1.0,
            "max_symbol_weight": 1.0,
            "selected_route_label": "pit_semantic_theme_r15:M01:leadership_soxl100_logit_v1",
    ...
    m02["name"] = "us_pit_semantic_theme_r15_m02"
    m02["description"] = (
    ...
    m02["portfolio"]["selected_route_label"] = "pit_semantic_theme_r15:M02:tail_exit_lgbm_v1"
    ...
    m02["notes"].update(
        {
            ...
            "fallback_candidate_id": "R15M01_then_R15D01",
            "low_confidence_action": "preserve_R15M01_target",
            "factor_library_ids": [
    ...
    m02["research_design"]["parameter_space"]["candidate_id"] = ["R15M02"]
    generated["R15M02"] = m02

    f01 = json.loads(json.dumps(m01))
    f01["name"] = "us_pit_semantic_theme_r15_f01"
    f01["description"] = "Exact no-semantic-input identity control for R15M01."
    f01["portfolio"]["selected_route_label"] = "pit_semantic_theme_r15:F01:exact_m01_fallback"
    f01["notes"].update(
        {
            "candidate_id": "R15F01",
            "candidate_role": "missing_modality_exact_m01_fallback",
            "method": "exact_r15m01_without_semantic_inputs_v1",
            "fallback_candidate_id": "R15M01_then_R15D01",
        }
    )
    f01["research_design"]["parameter_space"]["candidate_id"] = ["R15F01"]
    generated["R15F01"] = f01

    semantic_factor_names = {
        "R15C01": ("theme_confidence", "theme_direction"),
        "R15P01": ("shuffled_theme_direction",),
    }
    for candidate_id in ("R15C01", "R15P01"):
    ...
    generated["R15P01"]["notes"]["placebo_seed"] = 15108
    ...
    for candidate_id, raw in generated.items():
        ...
        if raw.get("execution_policy"):
            raw["execution_policy"]["policy_id"] = "r15_protected_opg_v1"
...
def _write_prompt(root: Path) -> None:
    source = root / "prompts/pit_semantic_theme_r14_factor_v1.txt"
...
def _write_source_cards(root: Path) -> None:
    ...
    for line in source.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        row = json.loads(line)
        retained.append(_replace_r14(row))
    retained.append(
        {
            "iteration_id": ITER_ID,
            "verification_status": "source_verified",
            "verified_at": "2026-08-05T18:14:18Z",
            "verification_method": "arxiv_official_api_metadata_and_abstract",
            "claim_id": "r15_ai_semiconductor_exuberance_heterogeneity",
            "claim": R15_NEW_SOURCE_CLAIM,
            "source_url": R15_NEW_SOURCE_URL,
            "source_type": "paper",
            ...
            "impact_on_spec": (
                "Retain price-confirmed leadership eligibility and the independent tail-risk "
                "diagnostic; do not treat AI-theme attention as an unconditional long signal."
            ),
            "limitations": (
                "This recent arXiv econometric study does not validate SOXL returns, R15 "
                "thresholds, execution costs, or any paper-trading decision."
            ),
        }
    )
    target.write_text(
...
def _customize_contract(
    ...
    if name == "data-contract.json":
        ...
        scope.update(
            {
                ...
                "allowed_candidate_ids": ["R15D01", "R15M01", "R15M02", "R15F01"],
                "limitations": [
        ...
        payload["single_hypothesis"] = (
            "concentrating the otherwise unchanged R14 semiconductor leadership target from "
            "80% SOXL plus 20% TQQQ to SOXL100 should increase upside without breaching the frozen "
        ...
        payload["capital"]["fallbacks"]["R15M02"] = "R15M01_then_R15D01"
        payload["deterministic_route"] = {
            "candidate_id": "R15D01",
            "route_label": "pit_semantic_theme_r15:D01:stable_leverage_state_v1",
        ...
        payload["quant_features"].update(
            {
                "candidate_ids": ["R15M01", "R15M02", "R15C01", "R15F01", "R15P01"],
                "families": [
        ...
        payload["pbo"]["selection_candidate_ids"] = ["R15D01", "R15M01", "R15M02"]
        payload["model_gates"] = {
            "R15M01": (
            ...
            "R15F01": "target and prediction identity with M01 for every matched session",
        }
        ...
        payload["semantic_gates"]["stock_budget_during_R15"] = 0.0
        ...
        payload.update(
            {
                "prior_effective_trial_count": 8062,
                "effective_trial_count": 8070,
                "prior_trial_evidence": {
                    "path": (
                        "reports/research/iterations/mom_pit_semantic_theme_r14/"
                        "historical-evaluation/evaluation-report.json"
                    ),
                    "status": "valid_clean_data_stop_price_paths_R14",
                },
            }
        )
    elif name == "universe-contract.json":
        payload["contract_id"] = "r15_dynamic_universe_v1"
                ...
                row.update(
                    {
                        ...
                        "candidates": ["R15D01", "R15M01", "R15C01", "R15F01", "R15P01"],
        ...
        payload["scout_network_status"] = {
            ...
            "action": (
                "verified arXiv 2604.12062 through the official API for current AI and "
                "semiconductor exuberance heterogeneity; it informs concentration risk only"
            ),
        }
...
def _model_reuse_round_decision(root: Path) -> dict[str, Any]:
    return {
        ...
        "reason": (
            "R14M01 reached 22.89% net CAGR with 74.51% TQQQ up-capture and used the "
            "80% SOXL plus 20% TQQQ enhancement for only 67 of 1008 development sessions. R15 "
            "changes that trained decision's target and label to SOXL100, so the R14 estimator "
            "cannot be reused; features, folds, cadence, and thresholds stay fixed"
        ),
...
def _write_external_brief(root: Path, specs: dict[str, Any], generated_at: str) -> None:
    payload = _replace_r14(_load_json(root / SOURCE_DIR / "external-brief.json"))
    payload.update(
        {
            "iter_id": ITER_ID,
            "strategy_name": specs["R15D01"].name,
            "objective": (
                "Test one concentration hypothesis against frozen R14 mechanics: QQQ in stress, "
                "TQQQ in normal risk-on, and SOXL100 only during confirmed semiconductor "
                "leadership. Fold-local ML may authorize that target or a QQQ tail exit; changing "
                "semantic themes remain forward-only and zero-capital."
            ),
            "source_spec_path": SPEC_PATHS["R15D01"].as_posix(),
            "spec_hash": strategy_content_hash(specs["R15D01"]),
            "current_source_card_paths": [SOURCE_CARD_PATH.as_posix()],
            "hypothesis_links": ["R15-H1"],
            "generated_at": generated_at,
        }
    )
    revisions = {row["candidate_id"]: row for row in payload["candidate_matrix_revisions"]}
    revisions["R15D01"]["revision"] = (
        "concentrate the frozen R14 leadership target from 80% SOXL plus 20% TQQQ to SOXL100"
    )
    revisions["R15M01"]["revision"] = (
        "retrain the fold-local Logistic classifier on the SOXL100-vs-TQQQ net label; "
        "low confidence is exact D01"
    )
    revisions["R15M02"]["revision"] = (
        "fold-local LightGBM path-survival diagnostic can only make high-confidence tail exits"
    )
    new_source = {
        "core_claim": R15_NEW_SOURCE_CLAIM,
        "credibility": "recent arXiv working paper metadata and abstract",
        "project_applicability": (
            "Treat semiconductor concentration as a conditional state with a separate tail-risk "
            "diagnostic, not as a permanent AI-theme allocation."
        ),
        "published_or_updated_at": "2026-05-09",
        "reflection": (
            "Bubble date-stamping evidence is not a return forecast, SOXL Alpha, or execution "
            "evidence."
        ),
        "source_type": "paper",
        "url": R15_NEW_SOURCE_URL,
    }
    payload["sources"].append(new_source)
    payload["source_evidence_bindings"].append(
        {
            "brief_claim_fingerprint": claim_fingerprint(R15_NEW_SOURCE_CLAIM),
            "canonical_url": R15_NEW_SOURCE_URL,
            "claim_fingerprint": claim_fingerprint(R15_NEW_SOURCE_CLAIM),
            "source_card_claim_id": "r15_ai_semiconductor_exuberance_heterogeneity",
        }
    )
    payload["topic_coverage"].append("AI_semiconductor_exuberance_heterogeneity")
...
def _write_markdown(root: Path) -> None:
    ...
    (iteration / "hypotheses.md").write_text(
        "\n".join(
            [
                "# Hypotheses: mom_pit_semantic_theme_r15",
                "",
                "## R15-H1",
                "",
                (
                    "- Hypothesis: R14 retained full Beta but its 80/20 leadership enhancement was "
                    "too diluted to meet the return and up-capture gates. Holding SOXL100 in the "
                    "same confirmed leadership state should increase upside; Logistic may "
                    "authorize only that target and LightGBM only a high-confidence QQQ exit."
                ),
                (
                ...
                (
                    "- Stop/Pivot criterion: do not tune any R15 threshold after results. Open the "
    ...
    (iteration / "search-space.md").write_text(
        "\n".join(
            [
                "# Search Space: mom_pit_semantic_theme_r15",
                "",
                (
    ...
    (iteration / "decision-record.md").write_text(
        "\n".join(
            [
                "# Decision Record: mom_pit_semantic_theme_r15",
                "",
                "## Preregistration",
                "",
                ...
                (
                    "- Reason: R14M01 stayed at 100% average risk exposure but reached only "
                    "22.89% net CAGR and 74.51% TQQQ up-capture; the 80/20 leadership target "
                    "appeared in only 67 of 1008 development sessions. R15 tests concentration "
                    "without changing confirmation, scheduling, thresholds, costs, or folds."
                ),
                (
    ...
    (iteration / "external-brief.md").write_text(
        "\n".join(
            [
                "# External Brief: mom_pit_semantic_theme_r15",
                "",
                (
                    "R15 reuses the verified evidence assembled through R14 on time-varying "
                    "product "
                    ...
                    "a concentrated SOXL100 semiconductor leadership state and QQQ tail exits, "
                    "repairs R14's return and up-capture shortfall? AI infrastructure and robotics "
                    "remain examples of current event-defined "
...
def _candidate_manifest(root: Path, specs: dict[str, Any]) -> dict[str, Any]:
    source = _replace_r14(_load_json(root / SOURCE_DIR / "candidate-manifest.json"))
    ...
    contract_names = {
        "data": ("r15_pit_semantic_theme_data_v1", "data-contract.json"),
        "features": ("r15_pit_semantic_theme_features_v1", "feature-contract.json"),
        "labels": ("r15_labels_v1", "label-contract.json"),
        "validation": ("r15_validation_v1", "validation-contract.json"),
        "costs": ("r15_costs_v1", "cost-contract.json"),
        "benchmarks": ("r15_benchmark_family_v1", "benchmark-contract.json"),
    }
    source["contracts"] = {
        group: {
            contract_id: {
    ...
    by_id["R15D01"].update(
        {
            ...
            "method": "qqq_tqqq_semiconductor_soxl100_state_v1",
    ...
    by_id["R15P01"]["placebo_seed"] = 15108
    return source
...
def _write_feasibility_and_search(root: Path, *, include_knowledge_assessment: bool) -> None:
    ...
    payload = _replace_r14(_load_json(root / SOURCE_DIR / "data-feasibility.json"))
    payload.update(
        {
            "report_type": "pit_semantic_theme_r15_data_feasibility",
            "iter_id": ITER_ID,
            "generated_at": datetime.now(UTC).isoformat(),
    ...
    search = _replace_r14(_load_json(root / SOURCE_DIR / "search-space.json"))
    search.update(
        {
            "iter_id": ITER_ID,
            "objective": (
                "Test the SOXL100 concentration change within frozen full-gross QQQ/TQQQ/SOXL "
                "states, a fold-local Logistic authorization model, a LightGBM QQQ tail-exit "
                "model, and forward-only "
    ...
    d01_spec = load_strategy_spec(root / SPEC_PATHS["R15D01"])
    search["source_spec_path"] = SPEC_PATHS["R15D01"].as_posix()
...
def _replace_r14(value: Any) -> Any:
    if isinstance(value, dict):
        return {_replace_text(str(key)): _replace_r14(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_replace_r14(item) for item in value]
...
def _replace_text(value: str) -> str:
    return value.replace("R14", "R15").replace("r14", "r15")
