...
SOURCE_ITER = "mom_pit_semantic_theme_r13"
ITER_ID = "mom_pit_semantic_theme_r14"
...
SOURCE_CARD_PATH = Path("reports/harness/source_cards/us_pit_semantic_theme_r14.jsonl")
SOURCE_SOURCE_CARD_PATH = Path("reports/harness/source_cards/us_pit_semantic_theme_r13.jsonl")
PROMPT_PATH = Path("prompts/pit_semantic_theme_r14_factor_v1.txt")
SPEC_PATHS = {
    candidate_id: Path(f"strategy_specs/drafts/us_pit_semantic_theme_r14_{suffix}.yaml")
...
def prepare(root: Path) -> None:
    ...
    if quality.get("status") != "ok" or quality.get("research_eligible") is not True:
        raise ValueError("R14 preparation requires a passing price-adjustment quality report")
    if snapshot.get("request_count") != 72:
        raise ValueError("R14 snapshot must bind 18 symbols x four adjustment modes")
    ...
    for name in CONTRACT_FILES:
        payload = _replace_r13(_load_json(base / SOURCE_DIR / name))
...
def finalize(root: Path) -> None:
    ...
    for name in (
        ...
        if not (iteration / name).is_file():
            raise ValueError(f"R14 finalization prerequisite is missing: {name}")
    assessment = _load_json(iteration / "knowledge-assessment.json")
    if assessment.get("status") != "ok":
        raise ValueError("R14 knowledge assessment must pass before finalization")
...
def _write_capability_reviews(root: Path) -> None:
    ...
    for candidate_id in ("R14D02", "R14L01", "R14C01", "R14P01"):
        ...
        payload = {
            ...
            "capabilities": [
                {
                    ...
                    "paper_ready": True,
                    "notes": (
                        "Daily bars are supported; R14 historical research binds an immutable "
            ...
            "blockers": [
                "new R14 forward epoch has no counted live semantic sessions",
    ...
    *,
    ...
    return {
        ...
        "params": {
            ...
            "resolver": "pit_semantic_theme_r14_panel_feature_v1",
        },
    }
...
def _write_specs(root: Path) -> dict[str, Any]:
    source_specs = {
        candidate_id.replace("R14", "R13", 1): yaml.safe_load(
            (
                root / str(path).replace("us_pit_semantic_theme_r14", "us_pit_semantic_theme_r13")
    ...
    for candidate_id in SPEC_PATHS:
        source_id = candidate_id.replace("R14", "R13", 1)
        raw = _replace_r13(source_specs[source_id])
        generated[candidate_id] = raw

    d01 = generated["R14D01"]
    d01["description"] = (
        "Low-turnover full-gross leverage state: hold QQQ in stress, TQQQ in normal Nasdaq "
        "risk-on, and a fixed SOXL/TQQQ barbell only during confirmed semiconductor leadership."
    )
    ...
    d01["portfolio"].update(
        {
            ...
            "selected_route_label": "pit_semantic_theme_r14:D01:stable_leverage_state_v1",
        }
    )
    d01["factors"] = {
        "qqq_momentum_120": {
        ...
        "soxl_momentum_20": {
            ...
            "default": 0.0,
            "description": "SOXL short-path confirmation before the leadership barbell.",
        },
        "soxl_trend_gap_100": {
            "source": "expression",
            "expression": "close / sma(close, 100) - 1",
            "default": 0.0,
            "description": "SOXL medium-trend confirmation before the leadership barbell.",
        },
    }
    ...
    d01["notes"].update(
        {
            "candidate_role": "deterministic_stable_full_gross_leverage_state",
            "method": "weekly_semiconductor_leadership_barbell_v1",
            "fallback_candidate_id": "QQQ100_stress_then_TQQQ100_normal",
            "factor_library_ids": [
            ...
            "route_contract": {
                ...
                "leadership_target": {"SOXL": 0.8, "TQQQ": 0.2},
    ...
    m01 = generated["R14M01"]
    m01["description"] = (
        "Fold-local Logistic classifier that may authorize only the fixed SOXL/TQQQ leadership "
        "barbell; low confidence preserves TQQQ or the deterministic QQQ stress target."
    )
    ...
    m01["portfolio"].update(
        {
            ...
            "gross_exposure_limit": 1.0,
            "max_symbol_weight": 1.0,
            "selected_route_label": "pit_semantic_theme_r14:M01:leadership_barbell_logit_v1",
    ...
    m01["notes"].update(
        {
            "candidate_role": "fold_local_leadership_barbell_classifier",
            "method": "logistic_semiconductor_barbell_vs_tqqq_v1",
            "fallback_candidate_id": "R14D01",
            "low_confidence_action": "D01_QQQ_stress_or_TQQQ_core",
            "factor_library_ids": [
                "core_beta_sleeve",
    ...
    m02["name"] = "us_pit_semantic_theme_r14_m02"
    m02["description"] = (
    ...
    m02["portfolio"]["selected_route_label"] = "pit_semantic_theme_r14:M02:tail_exit_lgbm_v1"
    ...
    m02["notes"].update(
        {
            ...
            "fallback_candidate_id": "R14M01_then_R14D01",
            "low_confidence_action": "preserve_R14M01_target",
            "factor_library_ids": [
    ...
    m02["research_design"]["parameter_space"]["candidate_id"] = ["R14M02"]
    generated["R14M02"] = m02

    f01 = json.loads(json.dumps(m01))
    f01["name"] = "us_pit_semantic_theme_r14_f01"
    f01["description"] = "Exact no-semantic-input identity control for R14M01."
    f01["portfolio"]["selected_route_label"] = "pit_semantic_theme_r14:F01:exact_m01_fallback"
    f01["notes"].update(
        {
            "candidate_id": "R14F01",
            "candidate_role": "missing_modality_exact_m01_fallback",
            "method": "exact_r14m01_without_semantic_inputs_v1",
            "fallback_candidate_id": "R14M01_then_R14D01",
        }
    )
    f01["research_design"]["parameter_space"]["candidate_id"] = ["R14F01"]
    generated["R14F01"] = f01

    semantic_factor_names = {
        "R14C01": ("theme_confidence", "theme_direction"),
        "R14P01": ("shuffled_theme_direction",),
    }
    for candidate_id in ("R14C01", "R14P01"):
    ...
    generated["R14P01"]["notes"]["placebo_seed"] = 14108
    ...
    for candidate_id, raw in generated.items():
        ...
        if raw.get("execution_policy"):
            raw["execution_policy"]["policy_id"] = "r14_protected_opg_v1"
...
def _write_prompt(root: Path) -> None:
    source = root / "prompts/pit_semantic_theme_r13_factor_v1.txt"
...
def _write_source_cards(root: Path) -> None:
    ...
    for line in source.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        row = json.loads(line)
        if row.get("claim_id") == "r13_ridge_finite_sample_bias_variance":
            continue
        retained.append(_replace_r13(row))
    new_cards = [
        {
            "iteration_id": ITER_ID,
            "verification_status": "source_verified",
            "verified_at": "2026-08-05T16:55:11Z",
            "verification_method": "arxiv_official_api_metadata_and_abstract",
            "claim_id": "r14_leveraged_etf_regime_compounding",
            "claim": (
                "A 2025 leveraged-ETF study reports that trends can enhance daily-reset "
                "compounding while mean reversion can cause underperformance, and that less "
                "frequent rebalancing can mitigate losses in mean-reverting regimes."
            ),
            "source_url": "https://arxiv.org/abs/2504.20116",
            "source_type": "paper",
            "accessed_at": "2026-08-05",
            "applies_to": ["leveraged_etf", "trend_momentum", "R14D01"],
            "impact_on_spec": (
                "Use persistent trend states and scheduled low-frequency target changes instead "
                "of daily winner reconstruction for the leveraged ETF core."
            ),
            "limitations": (
                "The paper is a working paper and does not validate R14 thresholds, returns, "
                "transaction costs, or semiconductor leadership selection."
            ),
        },
    ]
    target.write_text(
...
def _customize_contract(
    ...
    if name == "data-contract.json":
        ...
        scope.update(
            {
                ...
                "allowed_candidate_ids": ["R14D01", "R14M01", "R14M02", "R14F01"],
                "limitations": [
        ...
        payload["single_hypothesis"] = (
            "a scheduled full-gross QQQ_TQQQ_SOXL leverage state should retain beta while "
            "reducing R13 turnover; ML may only authorize the fixed semiconductor barbell or a "
            "rare tail exit, while semantic themes remain forward-only and zero-capital"
        )
        payload["semantic_features"]["prompt_path"] = PROMPT_PATH.as_posix()
        payload["capital"]["fallbacks"]["R14M02"] = "R14M01_then_R14D01"
        payload["deterministic_route"] = {
            "candidate_id": "R14D01",
            "route_label": "pit_semantic_theme_r14:D01:stable_leverage_state_v1",
        ...
        payload["quant_features"].update(
            {
                "candidate_ids": ["R14M01", "R14M02", "R14C01", "R14F01", "R14P01"],
                "families": [
        ...
        payload["model_roles"] = {
            "R14M01": {
                "kind": "logistic_regression_classifier",
                "decision": "authorize_fixed_SOXL80_TQQQ20_barbell_instead_of_TQQQ100",
        ...
        payload["placebo"]["seed"] = 14108
    elif name == "label-contract.json":
        payload["timing"].update({"purge_sessions": 21, "embargo_sessions": 21})
        payload["labels"]["R14M01"] = {
            "type": "fixed_semiconductor_barbell_beats_TQQQ_net_binary",
            "horizon_sessions": 20,
            "cost_bps": 20.0,
            "fit_scope": "fold_train_only",
            "positive_when": (
                "SOXL80_TQQQ20_open_to_open_return_minus_TQQQ_open_to_open_return_minus_"
                "incremental_switch_cost_is_positive; authorization_still_requires_leadership"
            ),
        }
        payload["labels"]["R14M02"] = {
        ...
        payload["pbo"]["selection_candidate_ids"] = ["R14D01", "R14M01", "R14M02"]
        payload["model_gates"] = {
            "R14M01": (
                "Logistic barbell authorization must improve net CAGR or MAR over D01 in at "
            ...
            "R14F01": "target and prediction identity with M01 for every matched session",
        }
        payload["route_integrity_gates"] = {
            "D01_default_risk_on_target_is_TQQQ100": True,
            "D01_stress_target_is_QQQ100": True,
            "D01_leadership_target_is_SOXL80_TQQQ20": True,
            "D01_average_risk_asset_exposure_floor": 0.99,
            "M01_low_confidence_uses_TQQQ_core_or_D01_QQQ_stress": True,
            "M02_non_tail_exact_M01": True,
            "review_every_sessions": 5,
            "minimum_hold_sessions": 10,
            "maximum_annualized_one_way_turnover": 12.0,
        }
        payload["semantic_gates"]["stock_budget_during_R14"] = 0.0
        ...
        payload.update(
            {
                "prior_effective_trial_count": 8054,
                "effective_trial_count": 8062,
                "prior_trial_evidence": {
                    "path": (
                        "reports/research/iterations/mom_pit_semantic_theme_r13/"
        ...
        payload["contract_id"] = "r14_dynamic_universe_v1"
        ...
        payload["fixed_execution_state_symbols"] = ["QQQ", "TQQQ", "SMH", "SOXL"]
                ...
                row.update(
                    {
                        ...
                        "candidates": ["R14D01", "R14M01", "R14C01", "R14F01", "R14P01"],
                        "method": "scheduled_leverage_state_then_logistic_barbell_authorization",
                        "fallback": "R14D01",
                    }
                )
                ...
                row.update(
                    {
                        ...
                        "candidates": ["R14D01", "R14M02"],
                        "method": "deterministic_trend_off_plus_fold_local_tail_survival",
                        "fallback": "R14M01_then_R14D01",
                    }
                )
            if row["role"] == "regime_meta_gate":
                row.update(
                    {
                        "candidates": ["R14D01", "R14M01", "R14M02"],
                        "method": "full_gross_QQQ_TQQQ_SOXL_state_with_rare_tail_exit",
                        "fallback": "R14D01",
                    }
                )
        for row in payload["required_ablations"]:
            if row["candidate"] == "R14M01":
                row["ablation"] = "quant_only_logistic_semiconductor_barbell"
            elif row["candidate"] == "R14M02":
        ...
        payload["round_decision"] = {
            ...
            "reason": (
                "R13 daily relative-strength switching produced 15-28x annual turnover and "
                "only 57-66% TQQQ up-capture; R14 changes the target state, horizon, labels, "
                "features, and retraining cadence, so no estimator can be reused"
            ),
        ...
        payload["scout_network_status"] = {
            ...
            "action": (
                "reuse verified R13 semantic and execution claims; add one leveraged-ETF "
                "regime-compounding source without treating it as alpha evidence"
            ),
        }
        payload["curated_candidates"] = [
            ...
            not in {
                "r14_ridge_finite_sample_bias_variance",
                "r14_leveraged_etf_regime_compounding",
            }
        ]
        payload["curated_candidates"].append(
            {
                "discovery_id": "r14_leveraged_etf_regime_compounding",
                "url": "https://arxiv.org/abs/2504.20116",
                "title": (
                    "Compounding Effects in Leveraged ETFs: Beyond the Volatility Drag Paradigm"
                ),
                "summary": (
                    "The paper reports that trend persistence can improve daily-reset leveraged-"
                    "ETF compounding, while mean reversion can produce underperformance and less "
                    "frequent rebalancing can mitigate losses in those regimes."
                ),
                "published_at": "2025-04-28T06:57:18Z",
                "authors": ["Chung-Han Hsieh", "Jow-Ran Chang", "Hui Hsiang Chen"],
                "source_type": "paper",
                "topics": ["leveraged ETF", "trend persistence", "rebalancing frequency"],
            }
        )


def _write_external_brief(root: Path, specs: dict[str, Any], generated_at: str) -> None:
    payload = _replace_r13(_load_json(root / SOURCE_DIR / "external-brief.json"))
    payload.update(
        {
            "iter_id": ITER_ID,
            "strategy_name": specs["R14D01"].name,
            "objective": (
                "Test one low-turnover full-gross leverage-state hypothesis: QQQ in stress, TQQQ "
                "in normal risk-on, and SOXL80/TQQQ20 only during confirmed semiconductor "
                "leadership. Fold-local ML may authorize that fixed barbell or a QQQ tail exit; "
                "changing semantic themes remain forward-only and zero-capital."
            ),
            "source_spec_path": SPEC_PATHS["R14D01"].as_posix(),
            "spec_hash": strategy_content_hash(specs["R14D01"]),
            "current_source_card_paths": [SOURCE_CARD_PATH.as_posix()],
            "hypothesis_links": ["R14-H1"],
            "generated_at": generated_at,
        }
    )
    revisions = {row["candidate_id"]: row for row in payload["candidate_matrix_revisions"]}
    revisions["R14D01"]["revision"] = (
        "replace daily multi-ETF winner switching with scheduled QQQ/TQQQ/SOXL-TQQQ leverage states"
    )
    revisions["R14M01"]["revision"] = (
        "fold-local Logistic classifier authorizes only the fixed semiconductor barbell; "
        "low confidence is exact D01"
    )
    revisions["R14M02"]["revision"] = (
    ...
    payload["source_evidence_bindings"] = [
        ...
        if row.get("source_card_claim_id") != "r14_ridge_finite_sample_bias_variance"
    ]
    ...
    additions = [
        {
            "url": "https://arxiv.org/abs/2504.20116",
            "claim_id": "r14_leveraged_etf_regime_compounding",
            "published_or_updated_at": "2025-04-28",
            "core_claim": (
                "A 2025 leveraged-ETF study reports that trends can enhance daily-reset "
                "compounding while mean reversion can cause underperformance, and that less "
                "frequent rebalancing can mitigate losses in mean-reverting regimes."
            ...
            "reflection": (
                "This working paper does not validate R14 thresholds, semiconductor leadership, "
                "transaction costs, or returns."
            ),
            "topic": "leveraged_etf_regime_compounding",
        }
    ]
...
def _write_markdown(root: Path) -> None:
    ...
    (iteration / "hypotheses.md").write_text(
        "\n".join(
            [
                "# Hypotheses: mom_pit_semantic_theme_r14",
                "",
                "## R14-H1",
                "",
                (
                    "- Hypothesis: R13 underperformed because daily winner switching incurred too "
                    "much cost and its defensive periods removed beta. A scheduled full-gross "
                    "QQQ/TQQQ/SOXL-TQQQ state can retain upside; Logistic may authorize only the "
                    "fixed semiconductor barbell and LightGBM only a high-confidence QQQ exit."
                ),
                (
                ...
                (
                    "- Stop/Pivot criterion: do not tune any R14 threshold after results. Open the "
    ...
    (iteration / "search-space.md").write_text(
        "\n".join(
            [
                "# Search Space: mom_pit_semantic_theme_r14",
                "",
                (
                    "Exactly eight preregistered roles are counted: deterministic high-beta "
                    "D01, deterministic semantic D02, Logistic barbell M01, LightGBM "
                    "tail-exit M02, "
    ...
    (iteration / "decision-record.md").write_text(
        "\n".join(
            [
                "# Decision Record: mom_pit_semantic_theme_r14",
                "",
                "## Preregistration",
                "",
                ...
                (
                    "- Reason: valid R13 diagnostics showed that the deterministic route averaged "
                    "81.3% to 82.7% risk exposure but still incurred 15-16x annual one-way "
                    "turnover and captured only 57-66% of TQQQ upside. R14 removes daily multi-ETF "
                    "ranking."
                ),
                (
                ...
                (
                    ...
                    "fixed SOXL80/TQQQ20 barbell beats TQQQ over 20 opens; M02 is a LightGBM "
    ...
    (iteration / "external-brief.md").write_text(
        "\n".join(
            [
                "# External Brief: mom_pit_semantic_theme_r14",
                "",
                (
                    "R14 reuses the verified evidence assembled through R13 on time-varying "
                    "product "
                    ...
                    "the default risk-on target, while permitting only scheduled "
                    "semiconductor leadership states and QQQ tail exits, repairs the beta and "
                    "turnover failures observed in "
                    "R13? AI infrastructure and robotics remain examples of current event-defined "
...
def _candidate_manifest(root: Path, specs: dict[str, Any]) -> dict[str, Any]:
    source = _replace_r13(_load_json(root / SOURCE_DIR / "candidate-manifest.json"))
    ...
    contract_names = {
        "data": ("r14_pit_semantic_theme_data_v1", "data-contract.json"),
        "features": ("r14_pit_semantic_theme_features_v1", "feature-contract.json"),
        "labels": ("r14_labels_v1", "label-contract.json"),
        "validation": ("r14_validation_v1", "validation-contract.json"),
        "costs": ("r14_costs_v1", "cost-contract.json"),
        "benchmarks": ("r14_benchmark_family_v1", "benchmark-contract.json"),
    }
    source["contracts"] = {
        group: {
            contract_id: {
    ...
    by_id["R14D01"].update(
        {
            "role": "deterministic_scheduled_full_gross_leverage_state",
            "method": "qqq_tqqq_semiconductor_barbell_state_v1",
            "ablation": "deterministic_scheduled_high_beta",
            "fallback": "QQQ100_stress_then_TQQQ100_normal",
        }
    )
    by_id["R14M01"].update(
        {
            "role": "fold_local_semiconductor_barbell_classifier",
            "method": "logistic_semiconductor_barbell_vs_tqqq_v1",
            "ablation": "quant_only_logistic_semiconductor_barbell",
            "fallback": "R14D01",
        }
    )
    by_id["R14M02"].update(
    ...
    by_id["R14P01"]["placebo_seed"] = 14108
    return source
...
def _write_feasibility_and_search(root: Path, *, include_knowledge_assessment: bool) -> None:
    ...
    payload = _replace_r13(_load_json(root / SOURCE_DIR / "data-feasibility.json"))
    payload.update(
        {
            "report_type": "pit_semantic_theme_r14_data_feasibility",
            "iter_id": ITER_ID,
            "generated_at": datetime.now(UTC).isoformat(),
    ...
    payload["data_scope"] = {
        ...
        "forward_news_capability": (
            "registered_live_collector_implemented; R13 observed 49 source packets but none "
            "count toward the new R14 epoch"
        ),
    ...
    for row in payload["candidate_authorization"]["rows"]:
        ...
        if row["candidate_id"] == "R14M01":
            row["reason_code"] = (
                "clean_SIP_features_available_for_fold_local_semiconductor_barbell_Logistic"
            )
        elif row["candidate_id"] == "R14M02":
    ...
    search = _replace_r13(_load_json(root / SOURCE_DIR / "search-space.json"))
    search.update(
        {
            "iter_id": ITER_ID,
            "objective": (
                "Test scheduled full-gross QQQ/TQQQ/SOXL-TQQQ states, a fold-local Logistic "
                "barbell authorization model, a LightGBM QQQ tail-exit model, and forward-only "
    ...
    d01_spec = load_strategy_spec(root / SPEC_PATHS["R14D01"])
    search["source_spec_path"] = SPEC_PATHS["R14D01"].as_posix()
    ...
    deterministic["parameters"].update(
        {
            "candidate_ids": ["R14D01"],
            "route_label": "pit_semantic_theme_r14:D01:stable_leverage_state_v1",
            "default_risk_on_target": "TQQQ100",
            "stress_target": "QQQ100",
            "leadership_target": "SOXL80_TQQQ20",
            "review_every_sessions": 5,
            "minimum_hold_sessions": 10,
    ...
    trained["parameters"].update(
        {
            "candidate_ids": ["R14M01", "R14M02"],
            "model_family": "Logistic_semiconductor_barbell_plus_LightGBM_QQQ_tail_exit",
            "fold_count": 4,
...
def _replace_r13(value: Any) -> Any:
    if isinstance(value, dict):
        return {_replace_text(str(key)): _replace_r13(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_replace_r13(item) for item in value]
...
def _replace_text(value: str) -> str:
    return value.replace("R13", "R14").replace("r13", "r14")
