...
RUNNER_PATH = Path("open_composer/research/pit_semantic_theme_r19.py")
TEST_PATH = Path("tests/test_pit_semantic_theme_r19.py")
SETUP_PATH = Path("scripts/prepare_pit_semantic_theme_r19.py")
...
PRICE_CANDIDATES = ("R19D01", "R19M01", "R19M02", "R19F01")
TRAINED_CANDIDATES = ("R19M01", "R19M02")
SEMANTIC_CANDIDATES = ("R19D02", "R19L01", "R19C01", "R19P01")
LOCK_STATUS = (
    "r19_clean_SIP_price_and_model_implementation_locked_before_first_candidate_evaluation"
)
SPEC_PATHS = {
...
def freeze_pit_semantic_theme_r19(root: Path) -> R19FreezeResult:
    base = root.resolve()
    lock_path = base / LOCK_PATH
    if lock_path.exists():
        raise ValueError("R19 historical evaluation lock already exists")
    if (base / OUTPUT_DIR).exists():
        raise ValueError("R19 historical evaluation exists before its lock")
    validation = validate_iteration_dossier(ITER_ID, root=base, stage="pre-backtest")
    if not validation.ok:
        raise ValueError("R19 pre-backtest dossier blocked: " + ", ".join(validation.blocked))
    specs = load_and_validate_r19_specs(base)
    panel = load_r19_price_panel(base)
    ...
    lock = {
        "schema_version": 1,
        "lock_contract": "pit_semantic_theme_r19_historical_evaluation_v1",
        "iter_id": ITER_ID,
        ...
        "m01_model_kind": specs["R19M01"].model.kind,
        "m02_model_kind": specs["R19M02"].model.kind,
...
def validate_r19_specs(specs: dict[str, StrategySpec]) -> None:
    if set(specs) != set(SPEC_PATHS):
        raise ValueError("R19 requires exactly eight preregistered specs")
    for candidate_id, spec in specs.items():
        ...
        if (
            ...
            raise ValueError(f"R19 spec safety, data, or universe mismatch: {candidate_id}")
    ...
    if (
        ...
        != "pit_semantic_theme_r19:D01:usd_core_stress_recovery_v1"
        ...
        raise ValueError("R19D01 deterministic route identity mismatch")

    for candidate_id in ("R19M01", "R19M02", "R19F01"):
        ...
        if (
            ...
            raise ValueError(f"R19 price-model portfolio contract mismatch: {candidate_id}")

    m01 = specs["R19M01"].model
    m02 = specs["R19M02"].model
    f01 = specs["R19F01"].model
    if m01 is None or m02 is None or f01 is None:
        raise ValueError("R19 model specifications are missing")
    common = (
    ...
    if not common:
        raise ValueError("R19 trained candidates do not share the matched contract")
    if (
        ...
        raise ValueError("R19 Logistic, tail-survival, or exact fallback identity mismatch")


def load_and_validate_r19_specs(root: Path) -> dict[str, StrategySpec]:
    specs = {
        candidate_id: load_strategy_spec(root / path) for candidate_id, path in SPEC_PATHS.items()
    }
    validate_r19_specs(specs)
    return specs


def load_r19_price_panel(root: Path) -> R11PricePanel:
    base = root.resolve()
    ...
    if (
        ...
        raise ValueError("R19 price-adjustment quality binding is not eligible")
    ...
    if set(by_symbol) != set(UNIVERSE):
        raise ValueError("R19 all-adjusted item coverage mismatch")
    ...
    if (
        ...
        raise ValueError("R19 common SIP session identity changed")
    panels = {
...
def development_folds(index: pd.DatetimeIndex) -> list[dict[str, Any]]:
    ...
    if len(selected) != 2136:
        raise ValueError("R19 development session capacity changed")
    fold_sessions = 252
    fold_count = 4
    purge = 21
    cursor = len(selected) - fold_count * fold_sessions
    if cursor - purge < 756:
        raise ValueError("R19 initial training capacity is insufficient")
    folds = []
    ...
    if cursor != len(selected):
        raise AssertionError("R19 fold allocation did not consume the development window")
    return folds
...
def _preflight(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R19 historical evaluation lock identity mismatch")
    for group in ("contracts", "specs", "implementation"):
        rows = lock.get(group)
        if not isinstance(rows, list) or not rows:
            raise ValueError(f"R19 lock group is missing: {group}")
        for row in rows:
    ...
    for candidate_id, spec in specs.items():
        if strategy_content_hash(spec) != locked_specs[candidate_id].get("semantic_sha256"):
            raise ValueError(f"R19 semantic spec changed after lock: {candidate_id}")
    validation = validate_iteration_dossier(ITER_ID, root=base, stage="pre-backtest")
    if not validation.ok:
        raise ValueError("R19 dossier changed after lock: " + ", ".join(validation.blocked))
    return {
        "lock": lock,
...
def _r19_frame_hash(frame: pd.DataFrame, columns: list[str]) -> str:
    ...
    if missing:
        raise ValueError(f"R19 frame hash columns are missing: {missing}")
    values = frame.loc[:, columns].astype(float).to_numpy(dtype="<f8", copy=True)
    if not np.isfinite(values).all():
        raise ValueError("R19 frame hash received nonfinite values")
    identities = [
...
def build_r19_feature_dataset(panel: R11PricePanel) -> pd.DataFrame:
    close = panel.close
    opens = panel.open
    ...
    if dataset.empty or not np.isfinite(dataset.loc[:, MODEL_FEATURES].to_numpy()).all():
        raise ValueError("R19 feature dataset is empty or nonfinite")
    return dataset


def training_rows_for_r19_prediction(
    ...
    *,
    ...
    if label_prefix is None:
        raise ValueError(f"R19 label is not registered: {label_name}")
...
def _fit_route_models(
    ...
    *,
    ...
    labels = {"R19M01": "m01_tqqq100_label", "R19M02": "m02_survival_label"}
    for candidate_id in TRAINED_CANDIDATES:
        spec = specs[candidate_id]
        assert spec.model is not None
        label_name = labels[candidate_id]
        train = training_rows_for_r19_prediction(
            dataset,
            decision_position=decision_position,
            spec=spec,
            label_name=label_name,
        )
        minimum_rows = 40 if candidate_id == "R19M01" else 80
        if len(train) < minimum_rows:
            raise ValueError(f"R19 has insufficient training rows: {candidate_id}")
        ...
        if train_y.nunique() < 2:
            ...
            fit_status = "fitted"
        label_prefix = "m01" if candidate_id == "R19M01" else "m02"
        record = {
            "schema_version": 1,
            "iter_id": ITER_ID,
            ...
            "training_data_sha256": _r19_frame_hash(train, [*MODEL_FEATURES, label_name]),
...
def _target_for_route(route: str) -> pd.Series:
    ...
    if route not in {STRESS_ROUTE, "QQQ", "TQQQ"}:
        raise ValueError(f"R19 route is not registered: {route}")
...
def _advance_nonstress_route(
    *,
    ...
    if desired_route not in {"TQQQ", "USD100"}:
        raise ValueError(f"R19 non-stress route is invalid: {desired_route}")
...
def build_r19_d01_targets(
    ...
    _validate_target_frame(frame, "R19D01")
...
def _predict_probability(model: FittedRouteModel, current: pd.DataFrame) -> float:
    ...
    if values.shape != (1, 2) or not np.isfinite(values).all():
        raise ValueError(f"R19 probability prediction is invalid: {model.candidate_id}")
    probability = float(values[0, 1])
    if not 0.0 <= probability <= 1.0:
        raise ValueError(f"R19 probability is out of bounds: {model.candidate_id}")
    return probability
...
def build_segment_targets(
    ...
    *,
    ...
    if points.empty:
        raise ValueError(f"R19 segment has no scheduled points: {segment_id}")
    target_rows = {candidate_id: [] for candidate_id in ("R19M01", "R19M02", "R19F01")}
    ...
    for point in points.to_dict(orient="records"):
        ...
        m01_probability = _predict_probability(fitted["R19M01"], current)
        m02_probability = _predict_probability(fitted["R19M02"], current)
        row = current.iloc[0]
        ...
        targets = {"R19M01": m01_target, "R19M02": m02_target, "R19F01": m01_target.copy()}
        prediction_base = {
            "schema_version": 1,
            "iter_id": ITER_ID,
            "segment_id": segment_id,
            "decision_session": str(row["decision_session"]),
            "execution_session": str(row["execution_session"]),
            "prediction_feature_sha256": _r19_frame_hash(current, list(MODEL_FEATURES)),
            **provenance,
        }
        for candidate_id, probability in (
            ("R19M01", m01_probability),
            ("R19M02", m02_probability),
        ):
        ...
        prediction_records.append(
            {
                **prediction_records[-2],
                "candidate_id": "R19F01",
                "fallback_source_candidate_id": "R19M01",
            }
        )
        ...
        for candidate_id, target in targets.items():
            ...
            weights = {symbol: float(target[symbol]) for symbol in UNIVERSE}
            model_id = (
                fitted["R19M01"].model_id
                if candidate_id == "R19F01"
            ...
            target_records.append(
                {
                    ...
                    "tail_exit_applied": tail_exit_applied if candidate_id == "R19M02" else False,
                    "weights": weights,
                    "target_sha256": _canonical_hash(weights),
                    "model_id": model_id,
                    "fallback_candidate_id": (
                        "R19M01" if candidate_id in {"R19M02", "R19F01"} else "R19D01"
                    ),
                    "fallback_applied": candidate_id == "R19F01",
                    **provenance,
                }
            )
    frames = {
    ...
    if not frames["R19F01"].equals(frames["R19M01"]):
        raise ValueError("R19F01 targets differ from R19M01")
...
def _validate_target_frame(frame: pd.DataFrame, candidate_id: str) -> None:
    if (
        ...
        raise ValueError(f"R19 target contract failed: {candidate_id}")


def run_pit_semantic_theme_r19(root: Path) -> R19EvaluationResult:
    base = root.resolve()
    output = base / OUTPUT_DIR
    if output.exists():
        raise ValueError("R19 historical evaluation is one-shot and already exists")
    ...
    if staging.exists() or any((base / ITERATION_DIR).glob("historical-evaluation.staging-*")):
        raise ValueError("R19 has unresolved historical-evaluation staging state")

    preflight = _preflight(base)
    specs = load_and_validate_r19_specs(base)
    panel = load_r19_price_panel(base)
    folds = development_folds(panel.open.index)
    dataset = build_r19_feature_dataset(panel)
    d01_targets, d01_records = build_r19_d01_targets(panel, specs["R19D01"], dataset)
    provenance = {
    ...
    if not aggregate_targets["R19F01"].equals(aggregate_targets["R19M01"]):
        raise ValueError("R19 aggregate fallback identity failed")

    start = pd.Timestamp(folds[0]["test_start"])
    end = pd.Timestamp(folds[-1]["test_end"])
    candidate_targets = {"R19D01": d01_targets, **aggregate_targets}
    ...
    pbo = cscv_probability_backtest_overfitting(
        {candidate_id: returns[candidate_id] for candidate_id in ("R19D01", "R19M01", "R19M02")},
        block_count=8,
    )
    ...
    development_pass_ids = [
        candidate_id
        for candidate_id in ("R19D01", "R19M01", "R19M02")
    ...
    payload = {
        "schema_version": 1,
        "report_type": "pit_semantic_theme_r19_historical_price_evaluation",
        "iter_id": ITER_ID,
        ...
        "integrity": {
            ...
            "f01_target_identity": aggregate_targets["R19F01"].equals(aggregate_targets["R19M01"]),
...
def _development_gates(
    ...
    d01_metrics = candidates["R19D01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    d01_folds = folds["R19D01"]
    m01_folds = folds["R19M01"]
    m02_folds = folds["R19M02"]
    m01_wins = sum(
    ...
    m02_metrics = candidates["R19M02"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    m01_metrics = candidates["R19M01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    ...
    output["R19D01"]["checks"].append(d01_route_gate)
    output["R19D01"]["pass"] = output["R19D01"]["pass"] and d01_route_gate["pass"]
    ...
    output["R19M01"]["checks"].append(m01_model_gate)
    output["R19M01"]["pass"] = output["R19M01"]["pass"] and m01_model_gate["pass"]
    output["R19M02"]["checks"].append(m02_model_gate)
    output["R19M02"]["pass"] = output["R19M02"]["pass"] and m02_model_gate["pass"]
    identity = _gate("F01_exact_M01_identity", True, True, True)
    output["R19F01"]["checks"].append(identity)
    output["R19F01"]["pass"] = False
    output["R19F01"]["selection_use"] = "identity_control_only"
    return output
...
def _model_comparison(
    ...
    logistic_tqqq_override = candidates["R19M01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    lightgbm_tail_exit = candidates["R19M02"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    fold_cagr = {
        candidate_id: [row["metrics"]["with_terminal"]["cagr"] for row in folds[candidate_id]]
        for candidate_id in ("R19M01", "R19M02")
    }
    return {
        "primary_question": (
        ...
        "lightgbm_tail_exit_positive_lift_folds": sum(
            ...
            for logistic_value, tail_exit_value in zip(
                fold_cagr["R19M01"], fold_cagr["R19M02"], strict=True
            )
        ),
    }
        ...
        rows.append(
            {
                ...
                "iter_id": ITER_ID,
                "candidate_id": candidate_id,
                "strategy_name": f"us_pit_semantic_theme_r19_{candidate_id[-3:].lower()}",
                ...
                "selection_eligible": candidate_id in {"R19D01", "R19M01", "R19M02"},
                "reason": (
...
def _render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# PIT Semantic Theme R19 Historical Price Evaluation",
        "",
    ...
    lines.extend(
        [
            "",
            "R19 uses a fresh multi-adjustment SIP snapshot. These are historical OOS "
