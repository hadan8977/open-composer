...
RUNNER_PATH = Path("open_composer/research/pit_semantic_theme_r14.py")
TEST_PATH = Path("tests/test_pit_semantic_theme_r14.py")
SETUP_PATH = Path("scripts/prepare_pit_semantic_theme_r14.py")
...
INCREMENTAL_OVERRIDE_COST = 0.0032
LEADERSHIP_WEIGHTS = {"SOXL": 0.8, "TQQQ": 0.2}
PRICE_CANDIDATES = ("R14D01", "R14M01", "R14M02", "R14F01")
TRAINED_CANDIDATES = ("R14M01", "R14M02")
SEMANTIC_CANDIDATES = ("R14D02", "R14L01", "R14C01", "R14P01")
LOCK_STATUS = (
    "r14_clean_SIP_price_and_model_implementation_locked_before_first_candidate_evaluation"
)
SPEC_PATHS = {
...
def freeze_pit_semantic_theme_r14(root: Path) -> R14FreezeResult:
    base = root.resolve()
    lock_path = base / LOCK_PATH
    if lock_path.exists():
        raise ValueError("R14 historical evaluation lock already exists")
    if (base / OUTPUT_DIR).exists():
        raise ValueError("R14 historical evaluation exists before its lock")
    validation = validate_iteration_dossier(ITER_ID, root=base, stage="pre-backtest")
    if not validation.ok:
        raise ValueError("R14 pre-backtest dossier blocked: " + ", ".join(validation.blocked))
    specs = load_and_validate_r14_specs(base)
    panel = load_r14_price_panel(base)
    ...
    lock = {
        "schema_version": 1,
        "lock_contract": "pit_semantic_theme_r14_historical_evaluation_v1",
        "iter_id": ITER_ID,
        ...
        "m01_model_kind": specs["R14M01"].model.kind,
        "m02_model_kind": specs["R14M02"].model.kind,
...
def validate_r14_specs(specs: dict[str, StrategySpec]) -> None:
    if set(specs) != set(SPEC_PATHS):
        raise ValueError("R14 requires exactly eight preregistered specs")
    for candidate_id, spec in specs.items():
        ...
        if (
            ...
            raise ValueError(f"R14 spec safety, data, or universe mismatch: {candidate_id}")

    d01 = specs["R14D01"]
    if (
        d01.portfolio.mode != "hybrid_adaptive_router"
        or d01.portfolio.selected_route_label
        != "pit_semantic_theme_r14:D01:stable_leverage_state_v1"
        or d01.portfolio.gross_exposure_limit != 1.0
        or d01.portfolio.max_symbol_weight != 1.0
        or d01.portfolio.max_symbols_per_day != 2
    ):
        raise ValueError("R14D01 deterministic route identity mismatch")

    for candidate_id in ("R14M01", "R14M02", "R14F01"):
        ...
        if (
            ...
            raise ValueError(f"R14 price-model portfolio contract mismatch: {candidate_id}")

    m01 = specs["R14M01"].model
    m02 = specs["R14M02"].model
    f01 = specs["R14F01"].model
    if m01 is None or m02 is None or f01 is None:
        raise ValueError("R14 model specifications are missing")
    common = (
    ...
    if not common:
        raise ValueError("R14 trained candidates do not share the matched contract")
    if (
        m01.kind != "logistic_regression_classifier"
        or m02.kind != "lightgbm_classifier"
        or m01.training.seed != 14101
        or m02.training.seed != 14102
        ...
        raise ValueError("R14 Logistic, tail-survival, or exact fallback identity mismatch")


def load_and_validate_r14_specs(root: Path) -> dict[str, StrategySpec]:
    specs = {
        candidate_id: load_strategy_spec(root / path) for candidate_id, path in SPEC_PATHS.items()
    }
    validate_r14_specs(specs)
    return specs


def load_r14_price_panel(root: Path) -> R11PricePanel:
    base = root.resolve()
    ...
    if (
        ...
        raise ValueError("R14 price-adjustment quality binding is not eligible")
    ...
    if set(by_symbol) != set(UNIVERSE):
        raise ValueError("R14 all-adjusted item coverage mismatch")
    ...
    if (
        ...
        raise ValueError("R14 common SIP session identity changed")
    panels = {
...
def development_folds(index: pd.DatetimeIndex) -> list[dict[str, Any]]:
    ...
    if len(selected) != 2136:
        raise ValueError("R14 development session capacity changed")
    fold_sessions = 252
    fold_count = 4
    purge = 21
    cursor = len(selected) - fold_count * fold_sessions
    if cursor - purge < 756:
        raise ValueError("R14 initial training capacity is insufficient")
    folds = []
    ...
    if cursor != len(selected):
        raise AssertionError("R14 fold allocation did not consume the development window")
    return folds
...
def _preflight(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R14 historical evaluation lock identity mismatch")
    for group in ("contracts", "specs", "implementation"):
        rows = lock.get(group)
        if not isinstance(rows, list) or not rows:
            raise ValueError(f"R14 lock group is missing: {group}")
        for row in rows:
    ...
    for candidate_id, spec in specs.items():
        if strategy_content_hash(spec) != locked_specs[candidate_id].get("semantic_sha256"):
            raise ValueError(f"R14 semantic spec changed after lock: {candidate_id}")
    validation = validate_iteration_dossier(ITER_ID, root=base, stage="pre-backtest")
    if not validation.ok:
        raise ValueError("R14 dossier changed after lock: " + ", ".join(validation.blocked))
    return {
        "lock": lock,
...
def _r14_frame_hash(frame: pd.DataFrame, columns: list[str]) -> str:
    ...
    if missing:
        raise ValueError(f"R14 frame hash columns are missing: {missing}")
    values = frame.loc[:, columns].astype(float).to_numpy(dtype="<f8", copy=True)
    if not np.isfinite(values).all():
        raise ValueError("R14 frame hash received nonfinite values")
    identities = [
...
def build_r14_feature_dataset(panel: R11PricePanel) -> pd.DataFrame:
    close = panel.close
    opens = panel.open
    ...
    breadth = (trend_100.loc[:, TECH_BREADTH_SYMBOLS] > 0.0).mean(axis=1)
    ...
    for position in range(200, len(close) - 1):
        ...
        semiconductor_leadership = bool(
            ...
            and momentum_20.iloc[position]["SOXL"] > 0.0
            and trend_100.iloc[position]["SOXL"] > 0.0
        )
        ...
        if has_m01_label:
            ...
            soxl_return = (
                float(opens.iloc[m01_label_end_position]["SOXL"])
                / float(opens.iloc[execution_position]["SOXL"])
                - 1.0
            )
            barbell_return = (
                LEADERSHIP_WEIGHTS["SOXL"] * soxl_return + LEADERSHIP_WEIGHTS["TQQQ"] * tqqq_return
            )
            m01_label = float(barbell_return - tqqq_return - INCREMENTAL_OVERRIDE_COST > 0.0)
        if has_m02_label:
            ...
            if semiconductor_leadership:
                soxl_path = opens["SOXL"].iloc[execution_position : m02_label_end_position + 1]
                soxl_wealth = soxl_path / float(soxl_path.iloc[0])
                wealth = (
                    LEADERSHIP_WEIGHTS["SOXL"] * soxl_wealth
                    + LEADERSHIP_WEIGHTS["TQQQ"] * tqqq_wealth
                )
            else:
                wealth = tqqq_wealth
        ...
        row: dict[str, Any] = {
            ...
            "soxl_momentum_20": float(momentum_20.iloc[position]["SOXL"]),
            "soxl_momentum_60": float(momentum_60.iloc[position]["SOXL"]),
            "soxl_trend_gap_100": float(trend_100.iloc[position]["SOXL"]),
            "soxl_drawdown_20": float(drawdown_20.iloc[position]["SOXL"]),
    ...
    if dataset.empty or not np.isfinite(dataset.loc[:, MODEL_FEATURES].to_numpy()).all():
        raise ValueError("R14 feature dataset is empty or nonfinite")
    return dataset


def training_rows_for_r14_prediction(
    ...
    *,
    ...
    label_prefix = {
        "m01_barbell_label": "m01",
        "m02_survival_label": "m02",
    }.get(label_name)
    if label_prefix is None:
        raise ValueError(f"R14 label is not registered: {label_name}")
...
def _fit_route_models(
    ...
    *,
    ...
    labels = {"R14M01": "m01_barbell_label", "R14M02": "m02_survival_label"}
    for candidate_id in TRAINED_CANDIDATES:
        ...
        minimum_rows = 40 if candidate_id == "R14M01" else 80
        if len(train) < minimum_rows:
            raise ValueError(f"R14 has insufficient training rows: {candidate_id}")
        ...
        if train_y.nunique() < 2:
            ...
            fit_status = "fitted"
        label_prefix = "m01" if candidate_id == "R14M01" else "m02"
        record = {
            "schema_version": 1,
            "iter_id": ITER_ID,
            ...
            "training_data_sha256": _r14_frame_hash(train, [*MODEL_FEATURES, label_name]),
...
def _target_for_route(route: str) -> pd.Series:
    if route == "SOXL80_TQQQ20":
        return _leadership_target()
    if route not in {"QQQ", "TQQQ"}:
        raise ValueError(f"R14 route is not registered: {route}")
...
def _advance_nonstress_route(
    *,
    ...
    if desired_route not in {"TQQQ", "SOXL80_TQQQ20"}:
        raise ValueError(f"R14 non-stress route is invalid: {desired_route}")
    if current_route in {None, "QQQ"}:
...
def build_r14_d01_targets(
    ...
    for item in dataset.to_dict(orient="records"):
        ...
        if not bool(row["risk_on"]):
            switched = held_route != "QQQ"
            held_sessions = held_sessions + 1 if held_route == "QQQ" else 1
            held_route = "QQQ"
            sessions_since_review = 0
            review_due = False
        else:
            desired = "SOXL80_TQQQ20" if bool(row["semiconductor_leadership"]) else "TQQQ"
    ...
    _validate_target_frame(frame, "R14D01")
...
def _predict_probability(model: FittedRouteModel, current: pd.DataFrame) -> float:
    ...
    if values.shape != (1, 2) or not np.isfinite(values).all():
        raise ValueError(f"R14 probability prediction is invalid: {model.candidate_id}")
    probability = float(values[0, 1])
    if not 0.0 <= probability <= 1.0:
        raise ValueError(f"R14 probability is out of bounds: {model.candidate_id}")
    return probability
...
def build_segment_targets(
    ...
    *,
    ...
    if points.empty:
        raise ValueError(f"R14 segment has no scheduled points: {segment_id}")
    target_rows = {candidate_id: [] for candidate_id in ("R14M01", "R14M02", "R14F01")}
    ...
    for point in points.to_dict(orient="records"):
        ...
        m01_probability = _predict_probability(fitted["R14M01"], current)
        m02_probability = _predict_probability(fitted["R14M02"], current)
        row = current.iloc[0]
        if not bool(row["risk_on"]):
            m01_switched = m01_held_route != "QQQ"
            m01_held_sessions = m01_held_sessions + 1 if m01_held_route == "QQQ" else 1
            m01_held_route = "QQQ"
            m01_sessions_since_review = 0
        ...
        if m01_held_route != "QQQ" and tail_exit_sessions > 0:
        ...
        targets = {"R14M01": m01_target, "R14M02": m02_target, "R14F01": m01_target.copy()}
        prediction_base = {
            "schema_version": 1,
            "iter_id": ITER_ID,
            "segment_id": segment_id,
            "decision_session": str(row["decision_session"]),
            "execution_session": str(row["execution_session"]),
            "prediction_feature_sha256": _r14_frame_hash(current, list(MODEL_FEATURES)),
            **provenance,
        }
        for candidate_id, probability in (
            ("R14M01", m01_probability),
            ("R14M02", m02_probability),
        ):
        ...
        prediction_records.append(
            {
                **prediction_records[-2],
                "candidate_id": "R14F01",
                "fallback_source_candidate_id": "R14M01",
            }
        )
        ...
        for candidate_id, target in targets.items():
            ...
            weights = {symbol: float(target[symbol]) for symbol in UNIVERSE}
            model_id = (
                fitted["R14M01"].model_id
                if candidate_id == "R14F01"
            ...
            target_records.append(
                {
                    ...
                    "execution_session": str(row["execution_session"]),
                    "semiconductor_leadership": bool(row["semiconductor_leadership"]),
                    "m01_barbell_probability": m01_probability,
                    ...
                    "tail_exit_applied": tail_exit_applied if candidate_id == "R14M02" else False,
                    "weights": weights,
                    "target_sha256": _canonical_hash(weights),
                    "model_id": model_id,
                    "fallback_candidate_id": (
                        "R14M01" if candidate_id in {"R14M02", "R14F01"} else "R14D01"
                    ),
                    "fallback_applied": candidate_id == "R14F01",
                    **provenance,
                }
            )
    frames = {
    ...
    if not frames["R14F01"].equals(frames["R14M01"]):
        raise ValueError("R14F01 targets differ from R14M01")
...
def _validate_target_frame(frame: pd.DataFrame, candidate_id: str) -> None:
    if (
        ...
        raise ValueError(f"R14 target contract failed: {candidate_id}")


def run_pit_semantic_theme_r14(root: Path) -> R14EvaluationResult:
    base = root.resolve()
    output = base / OUTPUT_DIR
    if output.exists():
        raise ValueError("R14 historical evaluation is one-shot and already exists")
    ...
    if staging.exists() or any((base / ITERATION_DIR).glob("historical-evaluation.staging-*")):
        raise ValueError("R14 has unresolved historical-evaluation staging state")
    ...
    d01_targets, d01_records = build_r14_d01_targets(panel, specs["R14D01"], dataset)
    provenance = {
    ...
    if not aggregate_targets["R14F01"].equals(aggregate_targets["R14M01"]):
        raise ValueError("R14 aggregate fallback identity failed")

    start = pd.Timestamp(folds[0]["test_start"])
    end = pd.Timestamp(folds[-1]["test_end"])
    candidate_targets = {"R14D01": d01_targets, **aggregate_targets}
    ...
    pbo = cscv_probability_backtest_overfitting(
        {candidate_id: returns[candidate_id] for candidate_id in ("R14D01", "R14M01", "R14M02")},
        block_count=8,
    )
    ...
    development_pass_ids = [
        candidate_id
        for candidate_id in ("R14D01", "R14M01", "R14M02")
    ...
    payload = {
        "schema_version": 1,
        "report_type": "pit_semantic_theme_r14_historical_price_evaluation",
        "iter_id": ITER_ID,
        ...
        "integrity": {
            ...
            "f01_target_identity": aggregate_targets["R14F01"].equals(aggregate_targets["R14M01"]),
...
def _development_gates(
    ...
    d01_metrics = candidates["R14D01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    d01_folds = folds["R14D01"]
    m01_folds = folds["R14M01"]
    m02_folds = folds["R14M02"]
    m01_wins = sum(
    ...
    m02_metrics = candidates["R14M02"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    m01_metrics = candidates["R14M01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    ...
    output["R14D01"]["checks"].append(d01_route_gate)
    output["R14D01"]["pass"] = output["R14D01"]["pass"] and d01_route_gate["pass"]
    m01_model_gate = _gate(
        "Logistic_barbell_fold_lift_capture_and_turnover",
        m01_wins >= 3
    ...
    output["R14M01"]["checks"].append(m01_model_gate)
    output["R14M01"]["pass"] = output["R14M01"]["pass"] and m01_model_gate["pass"]
    output["R14M02"]["checks"].append(m02_model_gate)
    output["R14M02"]["pass"] = output["R14M02"]["pass"] and m02_model_gate["pass"]
    identity = _gate("F01_exact_M01_identity", True, True, True)
    output["R14F01"]["checks"].append(identity)
    output["R14F01"]["pass"] = False
    output["R14F01"]["selection_use"] = "identity_control_only"
    return output
...
def _model_comparison(
    ...
    logistic_barbell = candidates["R14M01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    lightgbm_tail_exit = candidates["R14M02"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    fold_cagr = {
        candidate_id: [row["metrics"]["with_terminal"]["cagr"] for row in folds[candidate_id]]
        for candidate_id in ("R14M01", "R14M02")
    }
    return {
        "primary_question": "does_LightGBM_tail_exit_justify_complexity_over_Logistic_barbell",
        "logistic_barbell": logistic_barbell,
        ...
        "lightgbm_tail_exit_positive_lift_folds": sum(
            ...
            for logistic_value, tail_exit_value in zip(
                fold_cagr["R14M01"], fold_cagr["R14M02"], strict=True
            )
        ),
    }
        ...
        rows.append(
            {
                ...
                "iter_id": ITER_ID,
                "candidate_id": candidate_id,
                "strategy_name": f"us_pit_semantic_theme_r14_{candidate_id[-3:].lower()}",
                ...
                "selection_eligible": candidate_id in {"R14D01", "R14M01", "R14M02"},
                "reason": (
...
def _render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# PIT Semantic Theme R14 Historical Price Evaluation",
        "",
    ...
    lines.extend(
        [
            "",
            "R14 uses a fresh multi-adjustment SIP snapshot. These are historical OOS "
