...
RUNNER_PATH = Path("open_composer/research/pit_semantic_theme_r13.py")
TEST_PATH = Path("tests/test_pit_semantic_theme_r13.py")
SETUP_PATH = Path("scripts/prepare_pit_semantic_theme_r13.py")
...
MODEL_FEATURES = (
    ...
    "override_advantage_20",
    "override_trend_gap_100",
    "risk_on_breadth",
)
OVERRIDE_SYMBOLS = ("QLD", "SOXL", "TECL", "ROM", "USD", "SMH", "SOXX", "XLK", "IGV")
MINIMUM_HOLD_SESSIONS = 3
SWITCH_SCORE_ADVANTAGE = 0.03
M01_OVERRIDE_THRESHOLD = 0.60
M02_SURVIVAL_EXIT_THRESHOLD = 0.35
INCREMENTAL_OVERRIDE_COST = 0.004
PRICE_CANDIDATES = ("R13D01", "R13M01", "R13M02", "R13F01")
TRAINED_CANDIDATES = ("R13M01", "R13M02")
SEMANTIC_CANDIDATES = ("R13D02", "R13L01", "R13C01", "R13P01")
LOCK_STATUS = (
    "r13_clean_SIP_price_and_model_implementation_locked_before_first_candidate_evaluation"
)
SPEC_PATHS = {
...
def freeze_pit_semantic_theme_r13(root: Path) -> R13FreezeResult:
    base = root.resolve()
    lock_path = base / LOCK_PATH
    if lock_path.exists():
        raise ValueError("R13 historical evaluation lock already exists")
    if (base / OUTPUT_DIR).exists():
        raise ValueError("R13 historical evaluation exists before its lock")
    validation = validate_iteration_dossier(ITER_ID, root=base, stage="pre-backtest")
    if not validation.ok:
        raise ValueError("R13 pre-backtest dossier blocked: " + ", ".join(validation.blocked))
    specs = load_and_validate_r13_specs(base)
    panel = load_r13_price_panel(base)
    ...
    lock = {
        "schema_version": 1,
        "lock_contract": "pit_semantic_theme_r13_historical_evaluation_v1",
        "iter_id": ITER_ID,
        ...
        "m01_model_kind": specs["R13M01"].model.kind,
        "m02_model_kind": specs["R13M02"].model.kind,
...
def validate_r13_specs(specs: dict[str, StrategySpec]) -> None:
    if set(specs) != set(SPEC_PATHS):
        raise ValueError("R13 requires exactly eight preregistered specs")
    for candidate_id, spec in specs.items():
        ...
        if (
            ...
            raise ValueError(f"R13 spec safety, data, or universe mismatch: {candidate_id}")

    d01 = specs["R13D01"]
    if (
        d01.portfolio.mode != "hybrid_adaptive_router"
        or d01.portfolio.selected_route_label
        != "pit_semantic_theme_r13:D01:persistent_beta_override_v1"
        or d01.portfolio.gross_exposure_limit != 1.0
        or d01.portfolio.max_symbol_weight != 1.0
        or d01.portfolio.max_symbols_per_day != 2
    ):
        raise ValueError("R13D01 deterministic route identity mismatch")

    for candidate_id in ("R13M01", "R13M02", "R13F01"):
        ...
        if (
            ...
            or portfolio.max_symbols_per_day != 2
            or portfolio.gross_exposure_limit != 1.0
            or portfolio.max_symbol_weight != 1.0
        ):
            raise ValueError(f"R13 price-model portfolio contract mismatch: {candidate_id}")

    m01 = specs["R13M01"].model
    m02 = specs["R13M02"].model
    f01 = specs["R13F01"].model
    if m01 is None or m02 is None or f01 is None:
        raise ValueError("R13 model specifications are missing")
    common = (
    ...
    if not common:
        raise ValueError("R13 trained candidates do not share the matched contract")
    if (
        m01.kind != "logistic_regression_classifier"
        or m02.kind != "lightgbm_classifier"
        or m01.training.seed != 13101
        or m02.training.seed != 13102
        or m02.label.max_drawdown_pct != 12.0
        or m02.label.min_terminal_return_pct != -5.0
        or f01.model_dump(mode="json") != m01.model_dump(mode="json")
    ):
        raise ValueError("R13 Logistic, tail-survival, or exact fallback identity mismatch")


def load_and_validate_r13_specs(root: Path) -> dict[str, StrategySpec]:
    specs = {
        candidate_id: load_strategy_spec(root / path) for candidate_id, path in SPEC_PATHS.items()
    }
    validate_r13_specs(specs)
    return specs


def load_r13_price_panel(root: Path) -> R11PricePanel:
    base = root.resolve()
    ...
    if (
        ...
        raise ValueError("R13 price-adjustment quality binding is not eligible")
    ...
    if set(by_symbol) != set(UNIVERSE):
        raise ValueError("R13 all-adjusted item coverage mismatch")
    ...
    if (
        ...
        raise ValueError("R13 common SIP session identity changed")
    panels = {
...
def development_folds(index: pd.DatetimeIndex) -> list[dict[str, Any]]:
    ...
    if len(selected) != 2136:
        raise ValueError("R13 development session capacity changed")
    fold_sessions = 252
    fold_count = 4
    purge = 10
    cursor = len(selected) - fold_count * fold_sessions
    if cursor - purge < 756:
        raise ValueError("R13 initial training capacity is insufficient")
    folds = []
    ...
    if cursor != len(selected):
        raise AssertionError("R13 fold allocation did not consume the development window")
    return folds
...
def _preflight(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R13 historical evaluation lock identity mismatch")
    for group in ("contracts", "specs", "implementation"):
        rows = lock.get(group)
        if not isinstance(rows, list) or not rows:
            raise ValueError(f"R13 lock group is missing: {group}")
        for row in rows:
    ...
    for candidate_id, spec in specs.items():
        if strategy_content_hash(spec) != locked_specs[candidate_id].get("semantic_sha256"):
            raise ValueError(f"R13 semantic spec changed after lock: {candidate_id}")
    validation = validate_iteration_dossier(ITER_ID, root=base, stage="pre-backtest")
    if not validation.ok:
        raise ValueError("R13 dossier changed after lock: " + ", ".join(validation.blocked))
    return {
        "lock": lock,
...
def _r13_frame_hash(frame: pd.DataFrame, columns: list[str]) -> str:
    identity_columns = ["decision_position", "decision_session", "execution_session"]
    for optional in ("label_end_position", "label_end_session", "eligible_override_symbol"):
        if optional in frame.columns:
            identity_columns.append(optional)
    missing = [name for name in [*identity_columns, *columns] if name not in frame.columns]
    if missing:
        raise ValueError(f"R13 frame hash columns are missing: {missing}")
    values = frame.loc[:, columns].astype(float).to_numpy(dtype="<f8", copy=True)
    if not np.isfinite(values).all():
        raise ValueError("R13 frame hash received nonfinite values")
    identities = [
...
def build_r13_feature_dataset(panel: R11PricePanel) -> pd.DataFrame:
    close = panel.close
    opens = panel.open
    ...
    trend_50 = close / close.rolling(50, min_periods=50).mean() - 1.0
    trend_100 = close / close.rolling(100, min_periods=100).mean() - 1.0
    ...
    scores = 0.6 * momentum_10 + 0.4 * momentum_20
    eligible = (
        (momentum_10.loc[:, OVERRIDE_SYMBOLS] > 0.0)
        & (momentum_20.loc[:, OVERRIDE_SYMBOLS] > 0.0)
        & (trend_100.loc[:, OVERRIDE_SYMBOLS] > 0.0)
        & (momentum_20.loc[:, OVERRIDE_SYMBOLS].sub(momentum_20["TQQQ"], axis=0) >= 0.02)
    )
    breadth = (trend_100.loc[:, OVERRIDE_SYMBOLS] > 0.0).mean(axis=1)
    ...
    for position in range(200, len(close) - 1):
        ...
        eligible_symbols = [
            symbol for symbol in OVERRIDE_SYMBOLS if bool(eligible.iloc[position][symbol])
        ]
        override_symbol = (
            sorted(
                eligible_symbols,
                key=lambda symbol: (-float(scores.iloc[position][symbol]), symbol),
            )[0]
            if eligible_symbols
            else None
        )
        feature_symbol = override_symbol or "TQQQ"
        label_end_position = position + 6
        has_label = label_end_position < len(opens)
        m01_label: float | None = None
        m02_label: float | None = None
        label_end_session: str | None = None
        if has_label:
            label_end_session = opens.index[label_end_position].date().isoformat()
            start_open = float(opens.iloc[execution_position]["TQQQ"])
            terminal_open = float(opens.iloc[label_end_position]["TQQQ"])
            tqqq_path = opens["TQQQ"].iloc[execution_position : label_end_position + 1]
            wealth = tqqq_path / start_open
            path_drawdown = wealth / wealth.cummax() - 1.0
            terminal_return = terminal_open / start_open - 1.0
            m02_label = float(path_drawdown.min() >= -0.12 and terminal_return >= -0.05)
            if override_symbol is not None:
                override_return = (
                    float(opens.iloc[label_end_position][override_symbol])
                    / float(opens.iloc[execution_position][override_symbol])
                    - 1.0
                )
                m01_label = float(
                    override_return - terminal_return - INCREMENTAL_OVERRIDE_COST > 0.0
                )
        risk_on = bool(
        ...
        row: dict[str, Any] = {
            ...
            "label_end_position": label_end_position if has_label else None,
            "label_end_session": label_end_session,
            "eligible_override_symbol": override_symbol,
            "eligible_override_score": (
                float(scores.iloc[position][override_symbol]) if override_symbol else None
            ),
            "tqqq_score": float(scores.iloc[position]["TQQQ"]),
            "risk_on": risk_on,
            "gld_defensive_ok": bool(
                momentum_60.iloc[position]["GLD"] > 0.0 and trend_50.iloc[position]["GLD"] > 0.0
            ),
            "m01_override_label": m01_label,
            ...
            "tqqq_momentum_5": float(momentum_5.iloc[position]["TQQQ"]),
            ...
            "override_momentum_10": float(momentum_10.iloc[position][feature_symbol]),
            "override_momentum_20": float(momentum_20.iloc[position][feature_symbol]),
            "override_advantage_20": float(
                momentum_20.iloc[position][feature_symbol] - momentum_20.iloc[position]["TQQQ"]
            ),
            "override_trend_gap_100": float(trend_100.iloc[position][feature_symbol]),
            "risk_on_breadth": float(breadth.iloc[position]),
        }
        for symbol in ("TQQQ", *OVERRIDE_SYMBOLS):
            row[f"score_{symbol}"] = float(scores.iloc[position][symbol])
            row[f"eligible_{symbol}"] = bool(
                symbol == "TQQQ" or eligible.iloc[position].get(symbol, False)
            )
        rows.append(row)
    dataset = pd.DataFrame(rows)
    if dataset.empty or not np.isfinite(dataset.loc[:, MODEL_FEATURES].to_numpy()).all():
        raise ValueError("R13 feature dataset is empty or nonfinite")
    return dataset


def training_rows_for_r13_prediction(
    ...
    *,
    ...
    train = dataset[
        ...
        & (dataset["label_end_position"] <= terminal_cutoff)
    ]
...
def _fit_route_models(
    ...
    *,
    ...
    labels = {"R13M01": "m01_override_label", "R13M02": "m02_survival_label"}
    for candidate_id in TRAINED_CANDIDATES:
        ...
        minimum_rows = 40 if candidate_id == "R13M01" else 80
        if len(train) < minimum_rows:
            raise ValueError(f"R13 has insufficient training rows: {candidate_id}")
        ...
        record = {
            ...
            "training_data_sha256": _r13_frame_hash(train, [*MODEL_FEATURES, label_name]),
...
def _target_weights(symbol: str, *, defensive_gld: bool = False) -> pd.Series:
...
def _defensive_target(row: pd.Series) -> pd.Series:
    return _target_weights("BIL", defensive_gld=bool(row["gld_defensive_ok"]))
...
def build_r13_d01_targets(
    ...
    for item in dataset.to_dict(orient="records"):
        ...
        if not bool(row["risk_on"]):
            target = _defensive_target(row)
            held_symbol = None
            held_sessions = 0
            selected = "GLD_BIL" if bool(row["gld_defensive_ok"]) else "BIL"
        else:
            desired = _optional_symbol(row["eligible_override_symbol"]) or "TQQQ"
            if held_symbol is None:
                held_symbol = desired
                held_sessions = 1
            elif held_symbol == desired:
                held_sessions += 1
            elif held_sessions >= MINIMUM_HOLD_SESSIONS:
                held_eligible = bool(row[f"eligible_{held_symbol}"])
                desired_score = float(row[f"score_{desired}"])
                held_score = float(row[f"score_{held_symbol}"])
                if not held_eligible or desired_score - held_score >= SWITCH_SCORE_ADVANTAGE:
                    held_symbol = desired
                    held_sessions = 1
                else:
                    held_sessions += 1
        ...
        records.append(
            {
                ...
                "eligible_override_symbol": row["eligible_override_symbol"],
    ...
    _validate_target_frame(frame, "R13D01")
...
def _predict_probability(model: FittedRouteModel, current: pd.DataFrame) -> float:
    ...
    if values.shape != (1, 2) or not np.isfinite(values).all():
        raise ValueError(f"R13 probability prediction is invalid: {model.candidate_id}")
    probability = float(values[0, 1])
    if not 0.0 <= probability <= 1.0:
        raise ValueError(f"R13 probability is out of bounds: {model.candidate_id}")
    return probability
...
def build_segment_targets(
    ...
    *,
    ...
    if points.empty:
        raise ValueError(f"R13 segment has no scheduled points: {segment_id}")
    target_rows = {candidate_id: [] for candidate_id in ("R13M01", "R13M02", "R13F01")}
    ...
    for point in points.to_dict(orient="records"):
        ...
        m01_probability = _predict_probability(fitted["R13M01"], current)
        m02_probability = _predict_probability(fitted["R13M02"], current)
        row = current.iloc[0]
        if not bool(row["risk_on"]):
            m01_target = _defensive_target(row)
            m01_held_symbol = None
            m01_held_sessions = 0
            tail_exit_sessions = 0
        else:
            override = _optional_symbol(row["eligible_override_symbol"])
            desired = (
                override
                if override is not None and m01_probability >= M01_OVERRIDE_THRESHOLD
                else "TQQQ"
            )
            if m01_held_symbol is None:
                m01_held_symbol = desired
                m01_held_sessions = 1
            elif m01_held_symbol == desired:
                m01_held_sessions += 1
            elif m01_held_sessions >= MINIMUM_HOLD_SESSIONS:
                m01_held_symbol = desired
                m01_held_sessions = 1
            else:
                m01_held_sessions += 1
            m01_target = _target_weights(m01_held_symbol)
            if m02_probability < M02_SURVIVAL_EXIT_THRESHOLD:
                tail_exit_sessions = MINIMUM_HOLD_SESSIONS
        if bool(row["risk_on"]) and tail_exit_sessions > 0:
            m02_target = _defensive_target(row)
            tail_exit_sessions -= 1
            tail_exit_applied = True
        else:
            m02_target = m01_target.copy()
            tail_exit_applied = False
        targets = {"R13M01": m01_target, "R13M02": m02_target, "R13F01": m01_target.copy()}
        prediction_base = {
            "schema_version": 1,
            "iter_id": ITER_ID,
            "segment_id": segment_id,
            "decision_session": str(row["decision_session"]),
            "execution_session": str(row["execution_session"]),
            "prediction_feature_sha256": _r13_frame_hash(current, list(MODEL_FEATURES)),
            **provenance,
        }
        for candidate_id, probability in (
            ("R13M01", m01_probability),
            ("R13M02", m02_probability),
        ):
        ...
        prediction_records.append(
            {
                **prediction_records[-2],
                "candidate_id": "R13F01",
                "fallback_source_candidate_id": "R13M01",
            }
        )
        ...
        for candidate_id, target in targets.items():
            ...
            weights = {symbol: float(target[symbol]) for symbol in UNIVERSE}
            model_id = (
                fitted["R13M01"].model_id
                if candidate_id == "R13F01"
            ...
            target_records.append(
                {
                    ...
                    "execution_session": str(row["execution_session"]),
                    "eligible_override_symbol": row["eligible_override_symbol"],
                    "m01_override_probability": m01_probability,
                    "m02_survival_probability": m02_probability,
                    "tail_exit_applied": tail_exit_applied if candidate_id == "R13M02" else False,
                    "weights": weights,
                    "target_sha256": _canonical_hash(weights),
                    "model_id": model_id,
                    "fallback_candidate_id": (
                        "R13M01" if candidate_id in {"R13M02", "R13F01"} else "R13D01"
                    ),
                    "fallback_applied": candidate_id == "R13F01",
                    **provenance,
                }
            )
    frames = {
    ...
    if not frames["R13F01"].equals(frames["R13M01"]):
        raise ValueError("R13F01 targets differ from R13M01")
...
def _validate_target_frame(frame: pd.DataFrame, candidate_id: str) -> None:
    if (
        ...
        raise ValueError(f"R13 target contract failed: {candidate_id}")


def run_pit_semantic_theme_r13(root: Path) -> R13EvaluationResult:
    base = root.resolve()
    output = base / OUTPUT_DIR
    if output.exists():
        raise ValueError("R13 historical evaluation is one-shot and already exists")
    ...
    if staging.exists() or any((base / ITERATION_DIR).glob("historical-evaluation.staging-*")):
        raise ValueError("R13 has unresolved historical-evaluation staging state")
    ...
    d01_targets, d01_records = build_r13_d01_targets(panel, specs["R13D01"], dataset)
    provenance = {
    ...
    if not aggregate_targets["R13F01"].equals(aggregate_targets["R13M01"]):
        raise ValueError("R13 aggregate fallback identity failed")

    start = pd.Timestamp(folds[0]["test_start"])
    end = pd.Timestamp(folds[-1]["test_end"])
    candidate_targets = {"R13D01": d01_targets, **aggregate_targets}
    ...
    pbo = cscv_probability_backtest_overfitting(
        {candidate_id: returns[candidate_id] for candidate_id in ("R13D01", "R13M01", "R13M02")},
        block_count=8,
    )
    ...
    development_pass_ids = [
        candidate_id
        for candidate_id in ("R13D01", "R13M01", "R13M02")
    ...
    payload = {
        "schema_version": 1,
        "report_type": "pit_semantic_theme_r13_historical_price_evaluation",
        "iter_id": ITER_ID,
        ...
        "integrity": {
            ...
            "f01_target_identity": aggregate_targets["R13F01"].equals(aggregate_targets["R13M01"]),
...
def _development_gates(
    ...
    d01_metrics = candidates["R13D01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    d01_folds = folds["R13D01"]
    m01_folds = folds["R13M01"]
    m02_folds = folds["R13M02"]
    m01_wins = sum(
    ...
    m02_metrics = candidates["R13M02"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    m01_metrics = candidates["R13M01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    ...
    output["R13D01"]["checks"].append(d01_route_gate)
    output["R13D01"]["pass"] = output["R13D01"]["pass"] and d01_route_gate["pass"]
    m01_model_gate = _gate(
        "Logistic_override_fold_lift_capture_and_turnover",
        m01_wins >= 3
    ...
    output["R13M01"]["checks"].append(m01_model_gate)
    output["R13M01"]["pass"] = output["R13M01"]["pass"] and m01_model_gate["pass"]
    output["R13M02"]["checks"].append(m02_model_gate)
    output["R13M02"]["pass"] = output["R13M02"]["pass"] and m02_model_gate["pass"]
    identity = _gate("F01_exact_M01_identity", True, True, True)
    output["R13F01"]["checks"].append(identity)
    output["R13F01"]["pass"] = False
    output["R13F01"]["selection_use"] = "identity_control_only"
    return output
...
def _model_comparison(
    ...
    logistic_override = candidates["R13M01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    lightgbm_tail_exit = candidates["R13M02"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    fold_cagr = {
    ...
    return {
        "primary_question": "does_LightGBM_tail_exit_justify_complexity_over_Logistic_override",
        "logistic_override": logistic_override,
        ...
        "lightgbm_tail_exit_positive_lift_folds": sum(
            ...
            for logistic_value, tail_exit_value in zip(
                fold_cagr["R13M01"], fold_cagr["R13M02"], strict=True
            )
        ),
    }
        ...
        rows.append(
            {
                ...
                "iter_id": ITER_ID,
                "candidate_id": candidate_id,
                "strategy_name": f"us_pit_semantic_theme_r13_{candidate_id[-3:].lower()}",
                ...
                "selection_eligible": candidate_id in {"R13D01", "R13M01", "R13M02"},
                "reason": (
...
def _render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# PIT Semantic Theme R13 Historical Price Evaluation",
        "",
    ...
    lines.extend(
        [
            "",
            "R13 uses a fresh multi-adjustment SIP snapshot. These are historical OOS "
