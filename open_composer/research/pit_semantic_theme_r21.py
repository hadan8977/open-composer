...
RUNNER_PATH = Path("open_composer/research/pit_semantic_theme_r21.py")
TEST_PATH = Path("tests/test_pit_semantic_theme_r21.py")
SETUP_PATH = Path("scripts/prepare_pit_semantic_theme_r21.py")
...
PRICE_CANDIDATES = ("R21D01", "R21M01", "R21M02", "R21F01")
TRAINED_CANDIDATES = ("R21M01", "R21M02")
SEMANTIC_CANDIDATES = ("R21D02", "R21L01", "R21C01", "R21P01")
LOCK_STATUS = (
    "r21_clean_SIP_price_and_model_implementation_locked_before_first_candidate_evaluation"
)
SPEC_PATHS = {
...
def freeze_pit_semantic_theme_r21(root: Path) -> R21FreezeResult:
    base = root.resolve()
    lock_path = base / LOCK_PATH
    if lock_path.exists():
        raise ValueError("R21 historical evaluation lock already exists")
    if (base / OUTPUT_DIR).exists():
        raise ValueError("R21 historical evaluation exists before its lock")
    validation = validate_iteration_dossier(ITER_ID, root=base, stage="pre-backtest")
    if not validation.ok:
        raise ValueError("R21 pre-backtest dossier blocked: " + ", ".join(validation.blocked))
    specs = load_and_validate_r21_specs(base)
    panel = load_r21_price_panel(base)
    ...
    lock = {
        "schema_version": 1,
        "lock_contract": "pit_semantic_theme_r21_historical_evaluation_v1",
        "iter_id": ITER_ID,
        ...
        "m01_model_kind": specs["R21M01"].model.kind,
        "m02_model_kind": specs["R21M02"].model.kind,
...
def validate_r21_specs(specs: dict[str, StrategySpec]) -> None:
    if set(specs) != set(SPEC_PATHS):
        raise ValueError("R21 requires exactly eight preregistered specs")
    for candidate_id, spec in specs.items():
        ...
        if (
            ...
            raise ValueError(f"R21 spec safety, data, or universe mismatch: {candidate_id}")
    ...
    if (
        ...
        != "pit_semantic_theme_r21:D01:usd_pressure_fast_recovery_v1"
        ...
        raise ValueError("R21D01 deterministic route identity mismatch")

    for candidate_id in ("R21M01", "R21M02", "R21F01"):
        ...
        if (
            ...
            raise ValueError(f"R21 price-model portfolio contract mismatch: {candidate_id}")

    m01 = specs["R21M01"].model
    m02 = specs["R21M02"].model
    f01 = specs["R21F01"].model
    if m01 is None or m02 is None or f01 is None:
        raise ValueError("R21 model specifications are missing")
    common = (
    ...
    if not common:
        raise ValueError("R21 trained candidates do not share the matched contract")
    if (
        ...
        raise ValueError("R21 Logistic, tail-survival, or exact fallback identity mismatch")


def load_and_validate_r21_specs(root: Path) -> dict[str, StrategySpec]:
    specs = {
        candidate_id: load_strategy_spec(root / path) for candidate_id, path in SPEC_PATHS.items()
    }
    validate_r21_specs(specs)
    return specs


def load_r21_price_panel(root: Path) -> R11PricePanel:
    base = root.resolve()
    ...
    if (
        ...
        raise ValueError("R21 price-adjustment quality binding is not eligible")
    ...
    if set(by_symbol) != set(UNIVERSE):
        raise ValueError("R21 all-adjusted item coverage mismatch")
    ...
    if (
        ...
        raise ValueError("R21 common SIP session identity changed")
    panels = {
...
def development_folds(index: pd.DatetimeIndex) -> list[dict[str, Any]]:
    ...
    if len(selected) != 2136:
        raise ValueError("R21 development session capacity changed")
    fold_sessions = 252
    fold_count = 4
    purge = 21
    cursor = len(selected) - fold_count * fold_sessions
    if cursor - purge < 756:
        raise ValueError("R21 initial training capacity is insufficient")
    folds = []
    ...
    if cursor != len(selected):
        raise AssertionError("R21 fold allocation did not consume the development window")
    return folds
...
def _preflight(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R21 historical evaluation lock identity mismatch")
    for group in ("contracts", "specs", "implementation"):
        rows = lock.get(group)
        if not isinstance(rows, list) or not rows:
            raise ValueError(f"R21 lock group is missing: {group}")
        for row in rows:
    ...
    for candidate_id, spec in specs.items():
        if strategy_content_hash(spec) != locked_specs[candidate_id].get("semantic_sha256"):
            raise ValueError(f"R21 semantic spec changed after lock: {candidate_id}")
    validation = validate_iteration_dossier(ITER_ID, root=base, stage="pre-backtest")
    if not validation.ok:
        raise ValueError("R21 dossier changed after lock: " + ", ".join(validation.blocked))
    return {
        "lock": lock,
...
def _r21_frame_hash(frame: pd.DataFrame, columns: list[str]) -> str:
    ...
    if missing:
        raise ValueError(f"R21 frame hash columns are missing: {missing}")
    values = frame.loc[:, columns].astype(float).to_numpy(dtype="<f8", copy=True)
    if not np.isfinite(values).all():
        raise ValueError("R21 frame hash received nonfinite values")
    identities = [
...
def _advance_usd_pressure(
    *,
    ...
    if not np.isfinite(values).all():
        raise ValueError("R21 USD pressure inputs must be finite")
    if active:
...
def build_r21_feature_dataset(panel: R11PricePanel) -> pd.DataFrame:
    close = panel.close
    opens = panel.open
    ...
    for position in range(200, len(close) - 1):
        ...
        if has_m01_label:
            ...
            m01_label = float(tqqq_return - usd_return - INCREMENTAL_OVERRIDE_COST > 0.0)
        if has_m02_label:
            route_symbol = "USD" if d01_route == "USD100" else d01_route
            route_path = opens[route_symbol].iloc[execution_position : m02_label_end_position + 1]
            wealth = route_path / float(route_path.iloc[0])
        ...
        row: dict[str, Any] = {
            ...
            "m01_tqqq100_label": m01_label,
            "m02_survival_label": m02_label,
            "m02_label_route": d01_route,
    ...
    if dataset.empty or not np.isfinite(dataset.loc[:, MODEL_FEATURES].to_numpy()).all():
        raise ValueError("R21 feature dataset is empty or nonfinite")
    return dataset


def training_rows_for_r21_prediction(
    ...
    *,
    ...
    label_prefix = {
        "m01_tqqq100_label": "m01",
        "m02_survival_label": "m02",
    }.get(label_name)
    if label_prefix is None:
        raise ValueError(f"R21 label is not registered: {label_name}")
...
def _fit_route_models(
    ...
    *,
    ...
    labels = {"R21M01": "m01_tqqq100_label", "R21M02": "m02_survival_label"}
    for candidate_id in TRAINED_CANDIDATES:
        spec = specs[candidate_id]
        assert spec.model is not None
        label_name = labels[candidate_id]
        train = training_rows_for_r21_prediction(
            dataset,
            decision_position=decision_position,
            spec=spec,
            label_name=label_name,
        )
        minimum_rows = 40 if candidate_id == "R21M01" else 80
        if len(train) < minimum_rows:
            raise ValueError(f"R21 has insufficient training rows: {candidate_id}")
        ...
        if train_y.nunique() < 2:
            ...
            fit_status = "fitted"
        label_prefix = "m01" if candidate_id == "R21M01" else "m02"
        record = {
            "schema_version": 1,
            "iter_id": ITER_ID,
            ...
            "training_data_sha256": _r21_frame_hash(train, [*MODEL_FEATURES, label_name]),
...
def _target_for_route(route: str) -> pd.Series:
    ...
    if route not in {STRESS_ROUTE, "QQQ", "TQQQ"}:
        raise ValueError(f"R21 route is not registered: {route}")
...
def _advance_nonstress_route(
    *,
    ...
    if desired_route not in {"TQQQ", "USD100"}:
        raise ValueError(f"R21 non-stress route is invalid: {desired_route}")
...
def build_r21_d01_targets(
    ...
    _validate_target_frame(frame, "R21D01")
...
def _predict_probability(model: FittedRouteModel, current: pd.DataFrame) -> float:
    ...
    if values.shape != (1, 2) or not np.isfinite(values).all():
        raise ValueError(f"R21 probability prediction is invalid: {model.candidate_id}")
    probability = float(values[0, 1])
    if not 0.0 <= probability <= 1.0:
        raise ValueError(f"R21 probability is out of bounds: {model.candidate_id}")
    return probability
...
def build_segment_targets(
    ...
    *,
    ...
    if points.empty:
        raise ValueError(f"R21 segment has no scheduled points: {segment_id}")
    target_rows = {candidate_id: [] for candidate_id in ("R21M01", "R21M02", "R21F01")}
    ...
    for point in points.to_dict(orient="records"):
        ...
        m01_probability = _predict_probability(fitted["R21M01"], current)
        m02_probability = _predict_probability(fitted["R21M02"], current)
        row = current.iloc[0]
        ...
        targets = {"R21M01": m01_target, "R21M02": m02_target, "R21F01": m01_target.copy()}
        prediction_base = {
            "schema_version": 1,
            "iter_id": ITER_ID,
            "segment_id": segment_id,
            "decision_session": str(row["decision_session"]),
            "execution_session": str(row["execution_session"]),
            "prediction_feature_sha256": _r21_frame_hash(current, list(MODEL_FEATURES)),
            **provenance,
        }
        for candidate_id, probability in (
            ("R21M01", m01_probability),
            ("R21M02", m02_probability),
        ):
        ...
        prediction_records.append(
            {
                **prediction_records[-2],
                "candidate_id": "R21F01",
                "fallback_source_candidate_id": "R21M01",
            }
        )
        ...
        for candidate_id, target in targets.items():
            ...
            weights = {symbol: float(target[symbol]) for symbol in UNIVERSE}
            model_id = (
                fitted["R21M01"].model_id
                if candidate_id == "R21F01"
            ...
            target_records.append(
                {
                    ...
                    "tail_exit_applied": tail_exit_applied if candidate_id == "R21M02" else False,
                    "weights": weights,
                    "target_sha256": _canonical_hash(weights),
                    "model_id": model_id,
                    "fallback_candidate_id": (
                        "R21M01" if candidate_id in {"R21M02", "R21F01"} else "R21D01"
                    ),
                    "fallback_applied": candidate_id == "R21F01",
                    **provenance,
                }
            )
    frames = {
    ...
    if not frames["R21F01"].equals(frames["R21M01"]):
        raise ValueError("R21F01 targets differ from R21M01")
...
def _validate_target_frame(frame: pd.DataFrame, candidate_id: str) -> None:
    if (
        ...
        raise ValueError(f"R21 target contract failed: {candidate_id}")


def run_pit_semantic_theme_r21(root: Path) -> R21EvaluationResult:
    base = root.resolve()
    output = base / OUTPUT_DIR
    if output.exists():
        raise ValueError("R21 historical evaluation is one-shot and already exists")
    ...
    if staging.exists() or any((base / ITERATION_DIR).glob("historical-evaluation.staging-*")):
        raise ValueError("R21 has unresolved historical-evaluation staging state")

    preflight = _preflight(base)
    specs = load_and_validate_r21_specs(base)
    panel = load_r21_price_panel(base)
    folds = development_folds(panel.open.index)
    dataset = build_r21_feature_dataset(panel)
    d01_targets, d01_records = build_r21_d01_targets(panel, specs["R21D01"], dataset)
    provenance = {
    ...
    if not aggregate_targets["R21F01"].equals(aggregate_targets["R21M01"]):
        raise ValueError("R21 aggregate fallback identity failed")

    start = pd.Timestamp(folds[0]["test_start"])
    end = pd.Timestamp(folds[-1]["test_end"])
    candidate_targets = {"R21D01": d01_targets, **aggregate_targets}
    ...
    pbo = cscv_probability_backtest_overfitting(
        {candidate_id: returns[candidate_id] for candidate_id in ("R21D01", "R21M01", "R21M02")},
        block_count=8,
    )
    ...
    development_pass_ids = [
        candidate_id
        for candidate_id in ("R21D01", "R21M01", "R21M02")
    ...
    payload = {
        "schema_version": 1,
        "report_type": "pit_semantic_theme_r21_historical_price_evaluation",
        "iter_id": ITER_ID,
        ...
        "integrity": {
            ...
            "f01_target_identity": aggregate_targets["R21F01"].equals(aggregate_targets["R21M01"]),
...
def _development_gates(
    ...
    d01_metrics = candidates["R21D01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    d01_folds = folds["R21D01"]
    m01_folds = folds["R21M01"]
    m02_folds = folds["R21M02"]
    m01_wins = sum(
    ...
    m02_metrics = candidates["R21M02"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    m01_metrics = candidates["R21M01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    ...
    output["R21D01"]["checks"].append(d01_route_gate)
    output["R21D01"]["pass"] = output["R21D01"]["pass"] and d01_route_gate["pass"]
    m01_model_gate = _gate(
        "Logistic_TQQQ_override_fold_lift_capture_and_turnover",
        m01_wins >= 3
    ...
    output["R21M01"]["checks"].append(m01_model_gate)
    output["R21M01"]["pass"] = output["R21M01"]["pass"] and m01_model_gate["pass"]
    output["R21M02"]["checks"].append(m02_model_gate)
    output["R21M02"]["pass"] = output["R21M02"]["pass"] and m02_model_gate["pass"]
    identity = _gate("F01_exact_M01_identity", True, True, True)
    output["R21F01"]["checks"].append(identity)
    output["R21F01"]["pass"] = False
    output["R21F01"]["selection_use"] = "identity_control_only"
    return output
...
def _model_comparison(
    ...
    logistic_tqqq_override = candidates["R21M01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    lightgbm_tail_exit = candidates["R21M02"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    fold_cagr = {
        candidate_id: [row["metrics"]["with_terminal"]["cagr"] for row in folds[candidate_id]]
        for candidate_id in ("R21M01", "R21M02")
    }
    return {
        "primary_question": (
            "does_LightGBM_tail_exit_justify_complexity_over_Logistic_TQQQ_override"
        ),
        "logistic_tqqq_override": logistic_tqqq_override,
        ...
        "lightgbm_tail_exit_positive_lift_folds": sum(
            ...
            for logistic_value, tail_exit_value in zip(
                fold_cagr["R21M01"], fold_cagr["R21M02"], strict=True
            )
        ),
    }
        ...
        rows.append(
            {
                ...
                "iter_id": ITER_ID,
                "candidate_id": candidate_id,
                "strategy_name": f"us_pit_semantic_theme_r21_{candidate_id[-3:].lower()}",
                ...
                "selection_eligible": candidate_id in {"R21D01", "R21M01", "R21M02"},
                "reason": (
...
def _render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# PIT Semantic Theme R21 Historical Price Evaluation",
        "",
    ...
    lines.extend(
        [
            "",
            "R21 uses a fresh multi-adjustment SIP snapshot. These are historical OOS "
