...
RUNNER_PATH = Path("open_composer/research/pit_semantic_theme_r16.py")
TEST_PATH = Path("tests/test_pit_semantic_theme_r16.py")
SETUP_PATH = Path("scripts/prepare_pit_semantic_theme_r16.py")
...
LEADERSHIP_WEIGHTS = {"SOXL": 1.0}
STRESS_ROUTE = "SQQQ"
PRICE_CANDIDATES = ("R16D01", "R16M01", "R16M02", "R16F01")
TRAINED_CANDIDATES = ("R16M01", "R16M02")
SEMANTIC_CANDIDATES = ("R16D02", "R16L01", "R16C01", "R16P01")
LOCK_STATUS = (
    "r16_clean_SIP_price_and_model_implementation_locked_before_first_candidate_evaluation"
)
SPEC_PATHS = {
...
def freeze_pit_semantic_theme_r16(root: Path) -> R16FreezeResult:
    base = root.resolve()
    lock_path = base / LOCK_PATH
    if lock_path.exists():
        raise ValueError("R16 historical evaluation lock already exists")
    if (base / OUTPUT_DIR).exists():
        raise ValueError("R16 historical evaluation exists before its lock")
    validation = validate_iteration_dossier(ITER_ID, root=base, stage="pre-backtest")
    if not validation.ok:
        raise ValueError("R16 pre-backtest dossier blocked: " + ", ".join(validation.blocked))
    specs = load_and_validate_r16_specs(base)
    panel = load_r16_price_panel(base)
    ...
    lock = {
        "schema_version": 1,
        "lock_contract": "pit_semantic_theme_r16_historical_evaluation_v1",
        "iter_id": ITER_ID,
        ...
        "m01_model_kind": specs["R16M01"].model.kind,
        "m02_model_kind": specs["R16M02"].model.kind,
...
def validate_r16_specs(specs: dict[str, StrategySpec]) -> None:
    if set(specs) != set(SPEC_PATHS):
        raise ValueError("R16 requires exactly eight preregistered specs")
    for candidate_id, spec in specs.items():
        ...
        if (
            ...
            raise ValueError(f"R16 spec safety, data, or universe mismatch: {candidate_id}")
    ...
    if (
        ...
        or d01_notes.get("route_contract", {}).get("leadership_target") != {"SOXL": 1.0}
        or d01_notes.get("route_contract", {}).get("stress_target") != {"SQQQ": 1.0}
    ):
        raise ValueError("R16D01 deterministic route identity mismatch")

    for candidate_id in ("R16M01", "R16M02", "R16F01"):
        ...
        if (
            ...
            raise ValueError(f"R16 price-model portfolio contract mismatch: {candidate_id}")

    m01 = specs["R16M01"].model
    m02 = specs["R16M02"].model
    f01 = specs["R16F01"].model
    if m01 is None or m02 is None or f01 is None:
        raise ValueError("R16 model specifications are missing")
    common = (
    ...
    if not common:
        raise ValueError("R16 trained candidates do not share the matched contract")
    if (
        ...
        raise ValueError("R16 Logistic, tail-survival, or exact fallback identity mismatch")


def load_and_validate_r16_specs(root: Path) -> dict[str, StrategySpec]:
    specs = {
        candidate_id: load_strategy_spec(root / path) for candidate_id, path in SPEC_PATHS.items()
    }
    validate_r16_specs(specs)
    return specs


def load_r16_price_panel(root: Path) -> R11PricePanel:
    base = root.resolve()
    ...
    if (
        ...
        raise ValueError("R16 price-adjustment quality binding is not eligible")
    ...
    if set(by_symbol) != set(UNIVERSE):
        raise ValueError("R16 all-adjusted item coverage mismatch")
    ...
    if (
        ...
        raise ValueError("R16 common SIP session identity changed")
    panels = {
...
def development_folds(index: pd.DatetimeIndex) -> list[dict[str, Any]]:
    ...
    if len(selected) != 2136:
        raise ValueError("R16 development session capacity changed")
    fold_sessions = 252
    fold_count = 4
    purge = 21
    cursor = len(selected) - fold_count * fold_sessions
    if cursor - purge < 756:
        raise ValueError("R16 initial training capacity is insufficient")
    folds = []
    ...
    if cursor != len(selected):
        raise AssertionError("R16 fold allocation did not consume the development window")
    return folds
...
def _preflight(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("R16 historical evaluation lock identity mismatch")
    for group in ("contracts", "specs", "implementation"):
        rows = lock.get(group)
        if not isinstance(rows, list) or not rows:
            raise ValueError(f"R16 lock group is missing: {group}")
        for row in rows:
    ...
    for candidate_id, spec in specs.items():
        if strategy_content_hash(spec) != locked_specs[candidate_id].get("semantic_sha256"):
            raise ValueError(f"R16 semantic spec changed after lock: {candidate_id}")
    validation = validate_iteration_dossier(ITER_ID, root=base, stage="pre-backtest")
    if not validation.ok:
        raise ValueError("R16 dossier changed after lock: " + ", ".join(validation.blocked))
    return {
        "lock": lock,
...
def _r16_frame_hash(frame: pd.DataFrame, columns: list[str]) -> str:
    ...
    if missing:
        raise ValueError(f"R16 frame hash columns are missing: {missing}")
    values = frame.loc[:, columns].astype(float).to_numpy(dtype="<f8", copy=True)
    if not np.isfinite(values).all():
        raise ValueError("R16 frame hash received nonfinite values")
    identities = [
...
def build_r16_feature_dataset(panel: R11PricePanel) -> pd.DataFrame:
    close = panel.close
    opens = panel.open
    ...
    for position in range(200, len(close) - 1):
        ...
        if has_m01_label:
            ...
            leadership_return = (
                LEADERSHIP_WEIGHTS["SOXL"] * soxl_return
    ...
    if dataset.empty or not np.isfinite(dataset.loc[:, MODEL_FEATURES].to_numpy()).all():
        raise ValueError("R16 feature dataset is empty or nonfinite")
    return dataset


def training_rows_for_r16_prediction(
    ...
    *,
    ...
    label_prefix = {
        "m01_soxl100_label": "m01",
        "m02_survival_label": "m02",
    }.get(label_name)
    if label_prefix is None:
        raise ValueError(f"R16 label is not registered: {label_name}")
...
def _fit_route_models(
    ...
    *,
    ...
    labels = {"R16M01": "m01_soxl100_label", "R16M02": "m02_survival_label"}
    for candidate_id in TRAINED_CANDIDATES:
        spec = specs[candidate_id]
        assert spec.model is not None
        label_name = labels[candidate_id]
        train = training_rows_for_r16_prediction(
            dataset,
            decision_position=decision_position,
            spec=spec,
            label_name=label_name,
        )
        minimum_rows = 40 if candidate_id == "R16M01" else 80
        if len(train) < minimum_rows:
            raise ValueError(f"R16 has insufficient training rows: {candidate_id}")
        ...
        if train_y.nunique() < 2:
            ...
            fit_status = "fitted"
        label_prefix = "m01" if candidate_id == "R16M01" else "m02"
        record = {
            "schema_version": 1,
            "iter_id": ITER_ID,
            ...
            "training_data_sha256": _r16_frame_hash(train, [*MODEL_FEATURES, label_name]),
...
def _target_for_route(route: str) -> pd.Series:
    ...
    if route not in {STRESS_ROUTE, "QQQ", "TQQQ"}:
        raise ValueError(f"R16 route is not registered: {route}")
...
def _advance_nonstress_route(
    *,
    ...
    if desired_route not in {"TQQQ", "SOXL100"}:
        raise ValueError(f"R16 non-stress route is invalid: {desired_route}")
...
def build_r16_d01_targets(
    ...
    for item in dataset.to_dict(orient="records"):
        ...
        if not bool(row["risk_on"]):
            switched = held_route != STRESS_ROUTE
            held_sessions = held_sessions + 1 if held_route == STRESS_ROUTE else 1
            held_route = STRESS_ROUTE
            sessions_since_review = 0
            review_due = False
        else:
            desired = "SOXL100" if bool(row["semiconductor_leadership"]) else "TQQQ"
    ...
    _validate_target_frame(frame, "R16D01")
...
def _predict_probability(model: FittedRouteModel, current: pd.DataFrame) -> float:
    ...
    if values.shape != (1, 2) or not np.isfinite(values).all():
        raise ValueError(f"R16 probability prediction is invalid: {model.candidate_id}")
    probability = float(values[0, 1])
    if not 0.0 <= probability <= 1.0:
        raise ValueError(f"R16 probability is out of bounds: {model.candidate_id}")
    return probability
...
def build_segment_targets(
    ...
    *,
    ...
    if points.empty:
        raise ValueError(f"R16 segment has no scheduled points: {segment_id}")
    target_rows = {candidate_id: [] for candidate_id in ("R16M01", "R16M02", "R16F01")}
    ...
    for point in points.to_dict(orient="records"):
        ...
        m01_probability = _predict_probability(fitted["R16M01"], current)
        m02_probability = _predict_probability(fitted["R16M02"], current)
        row = current.iloc[0]
        if not bool(row["risk_on"]):
            m01_switched = m01_held_route != STRESS_ROUTE
            m01_held_sessions = m01_held_sessions + 1 if m01_held_route == STRESS_ROUTE else 1
            m01_held_route = STRESS_ROUTE
            m01_sessions_since_review = 0
        ...
        targets = {"R16M01": m01_target, "R16M02": m02_target, "R16F01": m01_target.copy()}
        prediction_base = {
            "schema_version": 1,
            "iter_id": ITER_ID,
            "segment_id": segment_id,
            "decision_session": str(row["decision_session"]),
            "execution_session": str(row["execution_session"]),
            "prediction_feature_sha256": _r16_frame_hash(current, list(MODEL_FEATURES)),
            **provenance,
        }
        for candidate_id, probability in (
            ("R16M01", m01_probability),
            ("R16M02", m02_probability),
        ):
        ...
        prediction_records.append(
            {
                **prediction_records[-2],
                "candidate_id": "R16F01",
                "fallback_source_candidate_id": "R16M01",
            }
        )
        ...
        for candidate_id, target in targets.items():
            ...
            weights = {symbol: float(target[symbol]) for symbol in UNIVERSE}
            model_id = (
                fitted["R16M01"].model_id
                if candidate_id == "R16F01"
            ...
            target_records.append(
                {
                    ...
                    "execution_session": str(row["execution_session"]),
                    "semiconductor_leadership": bool(row["semiconductor_leadership"]),
                    "m01_soxl100_probability": m01_probability,
                    ...
                    "tail_exit_applied": tail_exit_applied if candidate_id == "R16M02" else False,
                    "weights": weights,
                    "target_sha256": _canonical_hash(weights),
                    "model_id": model_id,
                    "fallback_candidate_id": (
                        "R16M01" if candidate_id in {"R16M02", "R16F01"} else "R16D01"
                    ),
                    "fallback_applied": candidate_id == "R16F01",
                    **provenance,
                }
            )
    frames = {
    ...
    if not frames["R16F01"].equals(frames["R16M01"]):
        raise ValueError("R16F01 targets differ from R16M01")
...
def _validate_target_frame(frame: pd.DataFrame, candidate_id: str) -> None:
    if (
        ...
        raise ValueError(f"R16 target contract failed: {candidate_id}")


def run_pit_semantic_theme_r16(root: Path) -> R16EvaluationResult:
    base = root.resolve()
    output = base / OUTPUT_DIR
    if output.exists():
        raise ValueError("R16 historical evaluation is one-shot and already exists")
    ...
    if staging.exists() or any((base / ITERATION_DIR).glob("historical-evaluation.staging-*")):
        raise ValueError("R16 has unresolved historical-evaluation staging state")

    preflight = _preflight(base)
    specs = load_and_validate_r16_specs(base)
    panel = load_r16_price_panel(base)
    folds = development_folds(panel.open.index)
    dataset = build_r16_feature_dataset(panel)
    d01_targets, d01_records = build_r16_d01_targets(panel, specs["R16D01"], dataset)
    provenance = {
    ...
    if not aggregate_targets["R16F01"].equals(aggregate_targets["R16M01"]):
        raise ValueError("R16 aggregate fallback identity failed")

    start = pd.Timestamp(folds[0]["test_start"])
    end = pd.Timestamp(folds[-1]["test_end"])
    candidate_targets = {"R16D01": d01_targets, **aggregate_targets}
    ...
    pbo = cscv_probability_backtest_overfitting(
        {candidate_id: returns[candidate_id] for candidate_id in ("R16D01", "R16M01", "R16M02")},
        block_count=8,
    )
    ...
    development_pass_ids = [
        candidate_id
        for candidate_id in ("R16D01", "R16M01", "R16M02")
    ...
    payload = {
        "schema_version": 1,
        "report_type": "pit_semantic_theme_r16_historical_price_evaluation",
        "iter_id": ITER_ID,
        ...
        "integrity": {
            ...
            "f01_target_identity": aggregate_targets["R16F01"].equals(aggregate_targets["R16M01"]),
...
def _development_gates(
    ...
    d01_metrics = candidates["R16D01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    d01_folds = folds["R16D01"]
    m01_folds = folds["R16M01"]
    m02_folds = folds["R16M02"]
    m01_wins = sum(
    ...
    m02_metrics = candidates["R16M02"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    m01_metrics = candidates["R16M01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    ...
    output["R16D01"]["checks"].append(d01_route_gate)
    output["R16D01"]["pass"] = output["R16D01"]["pass"] and d01_route_gate["pass"]
    m01_model_gate = _gate(
        "Logistic_SOXL100_fold_lift_capture_and_turnover",
        m01_wins >= 3
    ...
    output["R16M01"]["checks"].append(m01_model_gate)
    output["R16M01"]["pass"] = output["R16M01"]["pass"] and m01_model_gate["pass"]
    output["R16M02"]["checks"].append(m02_model_gate)
    output["R16M02"]["pass"] = output["R16M02"]["pass"] and m02_model_gate["pass"]
    identity = _gate("F01_exact_M01_identity", True, True, True)
    output["R16F01"]["checks"].append(identity)
    output["R16F01"]["pass"] = False
    output["R16F01"]["selection_use"] = "identity_control_only"
    return output
...
def _model_comparison(
    ...
    logistic_soxl100 = candidates["R16M01"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    lightgbm_tail_exit = candidates["R16M02"]["windows"]["development_oos"]["primary_20bps"][
        "with_terminal"
    ]
    fold_cagr = {
    ...
    return {
        "primary_question": "does_LightGBM_tail_exit_justify_complexity_over_Logistic_SOXL100",
        "logistic_soxl100": logistic_soxl100,
        ...
        "lightgbm_tail_exit_positive_lift_folds": sum(
            ...
            for logistic_value, tail_exit_value in zip(
                fold_cagr["R16M01"], fold_cagr["R16M02"], strict=True
            )
        ),
    }
        ...
        rows.append(
            {
                ...
                "iter_id": ITER_ID,
                "candidate_id": candidate_id,
                "strategy_name": f"us_pit_semantic_theme_r16_{candidate_id[-3:].lower()}",
                ...
                "selection_eligible": candidate_id in {"R16D01", "R16M01", "R16M02"},
                "reason": (
...
def _render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# PIT Semantic Theme R16 Historical Price Evaluation",
        "",
    ...
    lines.extend(
        [
            "",
            "R16 uses a fresh multi-adjustment SIP snapshot. These are historical OOS "
