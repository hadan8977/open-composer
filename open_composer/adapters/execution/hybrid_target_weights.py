...
def _build_target_weight_rows(
    *,
    ...
    for index in range(start_index, end_index):
        ...
        per_symbol_weight = (
            min(max_symbol_weight, params.max_position_weight, gross_limit / len(selected))
            if selected
            else 0.0
        )
        target_by_symbol = {
            symbol: per_symbol_weight if symbol in selected else 0.0 for symbol in dataset.symbols
        }
