...
1. **Layout assessment:** inspect representative states and viewports. Use rendered or source evidence for the relevant questions:
...
Also inspect arbitrary spacing, overflow, stacking, and container behavior the detector cannot resolve. When a scan is used, combine it with the visual assessment before editing. A clean scan cannot prove hierarchy or rhythm.
...
Stop when the requested layout works. Use `polish` only for a remaining refinement within scope.
