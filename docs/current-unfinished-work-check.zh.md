...
- 当前文档目录已收敛为当前入口、当前检查、成熟化计划、审查方法和必要集成文档；历史审计和过期计划不再保留在 `docs/` 当前目录。
- `make verify` 已扩展为本地闭环门，覆盖 format/lint/test、repo check、capability evaluation、deploy prepare、Dashboard build/html/catalog、feature validate 和 readiness。
...
| 研究硬化 | 参数扫描、promotion report、leverage research、exposure switch research 已有 CLI、JSON/Markdown 报告和测试 | 继续只作为研究证据，不自动升 active / paper_auto |
...
| P2-2 | 研究验证硬化 | 参数扫描、backtest data sanity gate、promotion report、leverage research 和 exposure-switch research 已补；不是当前第一阻断 | 继续把这些研究报告通过 Dashboard 展示并保持最小闭环，不要把它扩展成完整机构级研究平台 |
