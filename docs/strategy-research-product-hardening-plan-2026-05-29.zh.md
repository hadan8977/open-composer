...
本计划方向正确，但原稿里有三处需要收紧，否则会重复造轮子或把研究态/生产态再次混在一起：

1. `origin/main` 已有 `ExperimentRun`、`SearchPolicy`、Factor Lab V2、regime/decay、market-data contract、execution-sim 和 Dashboard experiment read model。本计划不得新建第二套 run state；P1 的 progress jsonl 只能作为 `ExperimentRun` 的事件流补充，并在 latest summary 中引用。
2. `selected_route_label` 的放宽必须落在 `StrategySpec` 生命周期层，而不是完全移除约束。`draft` 可为空；`approved/active`、scan、target weights、paper readiness 仍必须强制有 route label。
3. rejected iteration 只能降低“后续 artifact 要求”，不能降低数据、来源、过拟合、未来函数等基础研究约束。`rejected` 是收口语义，不是绕过审计的快捷方式。

本次执行采用以下产品级硬约束：

- 所有新增命令必须写结构化 JSON + Markdown，且不依赖外部凭证才能生成 blocked artifact。
- Dashboard 只读取本地 artifact，不直接跑重任务。
- `strict_data` 不能因为有 data-compare artifact 就自动变 paper-ready；只有 comparison 明确 `ok` 才能降级，否则继续 blocked 但给出 remediation。
- route-level attribution 是 adaptive router 的 Factor Lab 等价证据；普通单标的/非 router 策略仍走 Factor Lab / Factor Lab V2。
- progress ledger 必须支持 partial failure snapshot，并与 `reports/experiments/index.jsonl` 的 run_id 可关联。
...
- 新增 `oc capability evaluate <spec>`，输出策略级 capability review artifact；同时保留 `oc capability test` 作为全 registry smoke test。
- 同步更新 `.agents/skills/data-capability-reviewer/SKILL.md`，避免 specialist skill 继续引用不存在或语义不清的命令。
...
- 不新建第二套 experiment state；`reports/research/runs/*.jsonl` 是事件流，`reports/experiments/index.jsonl` 仍是 run index。
- latest summary 必须包含对应 `experiment_run_id`，Dashboard 用同一个 read model 读。
...
- 对超时使用本地 runtime budget 检查，不引入 Ray/Optuna/scikit-learn 运行时依赖。
...
- 将 `PortfolioConfig` 的无条件 route label 校验移到 `StrategySpec` 生命周期校验。
- `lifecycle=draft` 的 router 可为空；`approved/active` 的 router 必须有 `selected_route_label`。
...
- `router_promotion._factor_lab_check()` 对 `adaptive_intraday_internal_router` 读取 `adaptive-factor-attribution`，对 `hybrid_adaptive_router` 保持读取既有 `hybrid-factor-attribution`。
...
- 复用已有 `open_composer.adapters.data.comparison.compare_ohlcv_sources`，不要新增第二套 OHLCV diff 引擎。
...
- 如果 comparison 通过，`strict_data` 可从 blocked 降为 warning 或 ok；如果 artifact 缺失或 blocked，`strict_data` 继续 blocked，但 details 必须包含 remediation。
...
- `StrategySpec` 增加 `universe_metadata` 显式字段；universe audit 优先读取该字段，再兼容读取 `notes.universe_audit`。
- 可选新增 `oc strategy fixed-universe <spec>` 写出 fixed-universe artifact。
...
- `candidate_status=rejected` 的 promotion/report artifact 不得作为 paper readiness evidence。
...
- 显示 latest progress event 和 partial/timeout 状态。
