...
状态：已完成第一版落地，等待外部 agent 复审

本次已落地范围：

- Wave 1：Research Mode 与 Experiment Index。
- Wave 2：Factor Lab V2 的截面非 ML 因子诊断第一版。
- Wave 3：SearchPolicy 与 Trial Governance 接入 parameter sweep。
- Wave 3A：逐 bar equity series、Regime Performance、Alpha Decay 与 Promotion research gate。
- Wave 4：Tick / L1 / L2 本地数据契约、manifest 和 fixture。
- Wave 5：基于 market data manifest 与 Nautilus backend plan 的 research-only execution simulation。
- Wave 6：CLI experiment artifacts/trace 与 Dashboard read model 的 experiment index 可观察性。
