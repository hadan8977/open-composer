...
状态：最小 promotion gate 已实现，并已接入 paper readiness。
...
- 生成 promotion report，明确是否允许进入 paper candidate，并把它接入 paper readiness。
...
- paper readiness 现在会检查 promotion report 是否 ready，没过门就不能进入 paper_auto。
