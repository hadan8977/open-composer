...
- ⚠️ 显示哪些可选 API key 还没配 (`oc setup keys` 帮你配,见 §3)
...
```bash
uv run oc capability test --fast
```
...
**只有需要 LLM review/draft 时才需要**。`oc strategy draft` 不带 `--use-llm` 时不需要。
...
详见 G2 完工后的 `config/notifications.yaml.example`。
...
### Q6: pytest 失败 7 个 (Nautilus / Alpaca / VPS bootstrap)
**与本地 dashboard 部署无关**:
- 3 个 Nautilus 测试 → Windows 上 NautilusTrader native lib 兼容问题,跑本地 dashboard 不需要
- 2 个 VPS bootstrap 测试 → 调用 `systemctl` 等 Linux-only 命令
- 2 个 Alpaca paper 测试 → 取决于环境,见 Issue tracker
...
| 想做 | 命令 |
|---|---|
| 草拟一个策略 | `uv run oc strategy draft "QQQ 15min breakout with volume filter"` |
| 跑一遍样本回测 | `uv run oc strategy backtest qqq_pullback_15m` |
| 看 paper readiness | `uv run oc paper readiness qqq_pullback_15m` |
| 跑 selector (regime-aware,需 G3 落地) | `uv run oc selector run example` |
| 部署到 VPS | 见 [docs/remote-dashboard-deploy.zh.md](./remote-dashboard-deploy.zh.md) |
| 配 Telegram 通知 (G2 落地后) | `uv run oc notify test --kind signal_actionable` |
...
Optional next steps:
  ...
  • Try a strategy: uv run oc strategy draft "..."
