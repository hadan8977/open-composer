...
Follow the [skill map](skill-map.md) and limit checks to the requested path. A detector result is a candidate observation requiring context, not proof of quality. Inspect the rendered experience and real interaction path.
...
Use an existing critique as one input. If it was stored by the bundled critique-storage tool and that workflow is in use, retrieve it with the launcher resolved from this skill directory (`impeccable.cmd` on Windows):
...
- Check representative affected viewports; expand only when a defect or the request requires broader coverage.
...
Confirm the affected path with applicable input methods. Select relevant checks rather than running a full device and state matrix:
...
- console errors, layout shift, interaction latency, and image loading in the changed path; relevant browser or native runtime behavior;
- agreement with DESIGN.md, neighboring features, and the user's scope.

Use applicable project checks and existing diagnostics as evidence. Run the relevant check after changes; do not bootstrap hooks or a detector solely to finish polish. Fix observed defects and explain unresolved limits. A clean scan does not replace visual judgment.
