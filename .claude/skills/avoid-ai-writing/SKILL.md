...
description: Use the detailed Avoid AI Writing rulebook when explicitly selected for a contextual prose audit or rewrite, voice profile, protected-content review, or edit-in-place task. Supports detect-only mode. The local general prose editor is no-ai-slop; this specialist does not determine AI authorship.
license: MIT
metadata:
  version: "3.35.0"
...
Follow the [local skill map](../impeccable/reference/skill-map.md). This detailed
rulebook remains explicitly invoked in the local configuration. For ordinary
draft editing use [no-ai-slop](../no-ai-slop/SKILL.md); do not stack both editors
