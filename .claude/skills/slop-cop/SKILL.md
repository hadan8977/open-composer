...
description: >-
  ...
  or code when the user requests scoring, a report-card ticket, or Slop Cop by
  name. Also supports explicitly requested audit and rewrite modes. Use
  no-ai-slop for ordinary prose edits and impeccable for general design review.
...
Follow the [local skill map](../impeccable/reference/skill-map.md). This is an optional graded-review specialist, not a mandatory final check on every design or draft. Preserve the design brief when reviewing UI, and do not run overlapping editors by default.
...
Use the mode the user requested. A bare “slop cop this” selects Ticket mode.
